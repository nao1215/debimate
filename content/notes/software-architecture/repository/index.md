...
前の 3 つが集約の出し入れで、`FindUnshipped` は業務の語をそのままメソッド名にしたものです。名前の付け方は後で扱います。インターフェースの宣言をドメイン層に置いた事が、依存の向きを決めています。`implements` を書く言語でも、実装がインターフェースを参照する向きは同じで、Go の場合はインターフェースを満たす型に宣言を要求しないため、実装のパッケージへ手を入れずに利用する場所で後から宣言できます。
