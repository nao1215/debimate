...
上図のコマンドは、`order.Cancel(...)` のようなメソッド呼び出しで表される状態変更の要求です。要求を受けた `Order` 集約がイベントを作り、イベントストアへ追記されます。イベントストアが唯一の一次記録で、`Order` の現在の状態はイベントの再生で、検索用の読み取りモデルはイベントの投影で、どちらも後から導出されます。
