...
イベントを他のシステムに配送する仕組み（Transactional Outbox など）は [Domain Event](../domain-event/) で説明済みのため繰り返しません。[CQRS](../cqrs/)（Command Query Responsibility Segregation、更新と読み取りでモデルを分ける考え方）の詳細も対象外とし、関係だけを末尾で整理します。
...
上記の図のコマンドは、`order.Cancel(...)` のようなメソッド呼び出しで表される状態変更の要求です。要求を受けた `Order` 集約がイベントを作り、イベントストアに追記されます。イベントストアが唯一の一次記録で、`Order` の現在の状態はイベントの再生で、検索用の読み取りモデルはイベントの投影で、どちらも後から導出されます。
...
「確定前の注文はキャンセルできない」のような検証は、イベントを新しく作る側のメソッド（Domain Event のノートの `Confirm` と同じ役割の部分）で行います。`apply` が受け取るのは検証を通って記録された過去の事実なので、業務規則の検証で失敗させる事はしません。失敗はイベントを作る側の責務です。この分離によって、イベントを作った時の業務規則をもう一度検証せずに、記録済みの事実を再生できます。
...
イベントが数千件に伸びると、毎回の再生が長くなります。対策は、一定の件数ごとに状態の写し（スナップショット）を保存し、直近のスナップショットとそれ以降のイベントだけで復元する事です。以下が復元の経路です。
...
「注文者ごとの購入金額の合計」のような集計は、ストリーム単位の再生では答えにくい質問です。答えるには、全ストリームを再生して状態を組み立てた上で、集計をやり直す事になります。そこで、イベントを順に読んで検索用のテーブルを作り続けるプロセス（projector）を置き、検索は projector が作ったテーブルに向けます。このテーブルが読み取りモデルです。
...
上記の図の projector がストリームを横断して読むために、全イベント共通の追記順の連番を version とは別に振る設計がよく使われます。ここでは、projector がイベントの追記とは別のタイミングで読み取りモデルに反映する構成を扱います。
