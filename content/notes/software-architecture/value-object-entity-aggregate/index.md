...
Value Object とは、属性の組み合わせだけで意味が決まり、同一性を持たないオブジェクトです。[Evans 氏の DDD Reference](https://www.domainlanguage.com/wp-content/uploads/2016/05/DDD_Reference_2015-03.pdf) は、この分類に対して「不変なものとして扱う」事を求めています。属性が全て等しければ等価で、変更が要る場合は新しい値を作ります。
...
`String` を 1 つ持たせただけで `Copy` は導出できなくなるため、Value Object の作り方によっては `&self` を取る形へ変える事になります。同じく、`Default` を導出リストへ足すと `Money::default()` がモジュールの外から呼べるようになり、Go のゼロ値と同じ穴が開きます。
...
Aggregate とは、1 つの単位として扱う一群のドメインオブジェクトで、その中の 1 つを集約ルートに選びます。[Martin Fowler 氏の説明](https://martinfowler.com/bliki/DDD_Aggregate.html)では、集約の外からの参照は集約ルートへ向かい、ルートが集約全体の整合性を保証します。読み書きの単位も集約になります。
...
上図のドメインイベントは、集約に何が起きたかを表すデータです。`OrderConfirmed` は「注文が確定した」を意味します。点線が避けたい経路で、2 つの集約を 1 つのトランザクションで更新すると、境界の意味が消え、ロックの範囲も広がります。

代わりにトランザクションを分けると、Order 集約の更新が終わってから Stock 集約へ反映が届くまでの間、2 つの状態がずれます。いずれ一致するものの、ある瞬間を切り取るとずれている状態を結果整合性と呼びます。扱い方は [Domain Event](../domain-event/) で詳しく見ます。

Evans 氏の DDD Reference も、集約の境界の内側では整合性の規則を同期的に適用し、境界をまたぐ更新は非同期に扱う、と書いています。ただしこれは原則で、Vernon 氏は破ってよい理由を 4 つ挙げています。判断の軸として Vernon 氏が示しているのは、そのデータを整合させるのが操作しているユーザ自身の仕事か、それとも別の誰かの仕事か、という問いです。
