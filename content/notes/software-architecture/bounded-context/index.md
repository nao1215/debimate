...
境界が必要なのは、DDD（Domain-Driven Design、ドメイン駆動設計）を採用しているかどうかとは関係がありません。1 つの語が組織の場所ごとに違う意味を持つ状況は、設計の流儀に関わらず現れます。境界を引かずに 1 つのモデルにまとめると、どの場所の言葉とも一致しないモデルが残ります。ここでは境界の引き方と、境界をまたぐ時の扱いに絞り、チーム編成や組織構造の設計には踏み込みません。
...
もう 1 つの手がかりが、変更の理由です。1 つの要求が 1 つの境界の中で完結するなら、その切り方は業務の形と合っています。1 つの要求が毎回 3 箇所に散るなら、境界が業務を横切っています。

境界が業務を横切っている徴候は、[Aggregate](../value-object-entity-aggregate/) にも出ます。Aggregate は、まとめて一貫性を保つオブジェクトの集まりで、Bounded Context の内側で組み立てる単位です。例えば、販売の値引き率と配送の追跡番号を 1 つの Aggregate に持たせていたら、どちらのユビキタス言語でも表せないモデルができています。
...
同じ資料では、モジュール（パッケージ）もモデルの一部として扱われ、その名前もユビキタス言語の一部にするとされています。そのため、モデルを表すコードでは、パッケージ名と型名がそのまま業務の語になります。

販売の境界を Go で書くと次のようになります。
...
| [Anticorruption Layer](../anti-corruption-layer/) | 下流 | 隔離層を挟み、相手のモデルを自分の語に翻訳する |
...
選び方は排他ではありません。上流が Open-host Service を出しつつ Published Language を定め、下流の一部は Conformist として従い、残りは Anticorruption Layer を作る、という組み合わせが起こります。

上流を動かせるなら、Customer/Supplier Development のように下流の要求を上流の計画に組み込めます。動かせない下流が自分だけで選ぶのは、Conformist か Anticorruption Layer です。

DDD Reference の 2 つの説明を並べると、上流のモデルに合わせる窮屈さを受け入れて統合の単純さを取るなら Conformist、上流のモデルが自分のプロジェクトの必要に合わず、合わせる事が現実的でないなら Anticorruption Layer になります。
...
上記の図の点線が避けたい経路です。相手のモデルを直接使うと、相手の語とデータの形が下流のモデルの内側まで届き、相手の変更に合わせて下流の設計が動く事になります。Anticorruption Layer を挟むと、相手が変わった時に直す場所が境界の入口 1 箇所で済みます。

---

### 境界は配置の単位ではない

注意点として、Bounded Context はプロセスやサービスの単位ではありません。1 つのプロセスの中に、複数の境界をパッケージやモジュールとして置けます。Fowler 氏も、1 つのアプリケーションの中に複数のコンテキストが現れる例として、メモリ上のモデルと関係 DB 上のモデルの分かれ方を挙げています。逆に、1 つの境界を複数のサービスに分けて実装する構成も取れます。
