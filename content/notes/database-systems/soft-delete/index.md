...
上図の会員 42 は、`orders` の行ごと消えています。指定を変えると何が守られ、何が失われるのかを以下に示します。

| `ON DELETE` の指定 | 会員を消した時の注文 | 失うもの |
| --- | --- | --- |
| `CASCADE` | 一緒に消える | 過去の注文そのもの |
| `RESTRICT` | 残る | 「退会した会員を消す」操作そのものが成立しない |
| `SET NULL` | 残る（`member_id` は `NULL`） | 注文から会員を辿る経路 |

どの指定を選んでも、会員の行を消す限り何かは失われます。消えて困る理由は、代表的には 3 つあります。

- 上図のように、確定していた集計や帳票の値が後から変わる
...
上図の匿名化を選べるかどうかは、要件を満たす形で不可逆にできるかで決まります。どこまで匿名化すれば足りるのかは、扱うデータと法的な要件によって変わります。
