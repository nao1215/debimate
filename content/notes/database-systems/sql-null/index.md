...
- 書いた条件どおりに行が絞られない事があるのは、この第 3 の結果が式の全体へ伝わり、`NOT` や `NOT IN` を通った先で直感と違う結果になるためです
...
本ノートでは、SQL が `NULL` をどう扱うかと、その扱いが表の設計へ与える影響を扱い、プログラミング言語の null 参照や JSON の null は対象外とします。`NULL` の扱いには SQL 標準が定めている部分と、DBMS（Database Management System）ごとに決めてよい部分があるので、標準の規則を軸に置き、実装で割れる所は都度断ります。
