...
この規則は、接続ライブラリの設定項目や `SET TRANSACTION ISOLATION LEVEL` という文として表に出ます。分離が効いてくる場面の代表は、在庫の引き当てです。残り 1 個の商品へ 2 件の注文がほぼ同時に届くと、どちらの処理も残数として 1 を読み、そこから 1 を引いた 0 を書きます。読んだ値をアプリケーション側で計算して書き戻す形なので、DB から見れば 2 件とも正しい更新です。
