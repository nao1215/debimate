...
通信量だけは、ノード数に対する増え方が仕組みで違います。

![ノード数に対するメッセージ本数を両対数で比べた図。相互監視の 1 周期あたり O(n^2) が最も急に伸び、gossip で情報 1 件が全体へ届くまでの O(n log n)、中央集約と Lease の 1 周期あたり O(n) が続く](images/heartbeat_message_scaling.svg)

上記の図は係数を置いていないので、読むのは目盛りの値ではなく傾きです。相互監視だけが違う傾きで伸び、ノードが増えるほど他の 2 本から離れていきます。Phi Accrual が流す信号はハートビートと同じものなので、通信量も同じです。
