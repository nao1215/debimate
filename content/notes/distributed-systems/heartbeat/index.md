...
上記の図の点線部分が途絶えた信号で、Monitor に「故障しました」という報告は届きません。落ちたノードは自分の停止を報告できないので、Monitor は「何も届かない事」から Node 3 の故障を判断します。
...
上記の図の 3 つのケースは原因も対処も違うのに、Monitor から見える現象は同じです。非同期ネットワーク（メッセージの遅延に上限を置かないモデル）では、何秒待っても「まだ遅いだけ」という可能性が残るので、「遅いだけのノード」と「落ちたノード」を確実に見分けられません。これは実装の工夫で解決できません。
...
監視の形は 2 通りあります。1 台の監視役が全ノードを見る中央集約型（central monitoring）と、各ノードが互いを見る相互監視型（mutual monitoring）です。ノード数を `n` とすると、1 周期に流れるメッセージは中央集約型で `n` 本、相互監視型で `n × (n-1)` 本になります。中央集約型は通信量が少ない代わりに、監視役に負荷が集中し、そこが落ちると誰も故障を検知できません。相互監視型は特定の監視役に依存しない一方で、ノードが増えると通信量が急に膨らみます。
...
過半数を要求する構成では、少数派は票を集められないためリーダーが立ちません。それでも、分断された旧リーダーは、交代に気付くまで自分をリーダーだと思い込んだまま動き続けます。書き込みの確定にも過半数の応答を要求する構成なら、旧リーダーは書き込みを確定できません。しかし、古い値を読み取りに返す可能性は残ります。

Raft では、リーダーが読み取りに応答する前に過半数とハートビートを交換し、自分がまだリーダーである事を確認します。この手順は [Leader and Followers](../leader-and-followers/) のノートで扱っています。代わりに、ハートビートを [Lease](../lease/) のように扱う方法もあります（[Raft の論文](https://raft.github.io/raft.pdf) 8 節）。Lease は「いつまで権利があるか」を借りた側も判断できる仕組みで、ノード間で時計の進む速さのずれに上限がある事を前提にします。

監視側が最後の受信からの経過時間を測る時は、単調増加する時計（monotonic clock）を使います。壁時計は、時刻をネットワーク越しに合わせる NTP（Network Time Protocol）の補正で進んだり巻き戻ったりします。そのまま差分を取ると、補正の分だけ故障判定が早まったり遅れたりします。
...
- 相互監視の構成では、1 周期あたりのメッセージが `n × (n-1)` 本に膨らむ
