...
同時に 2 つの書き込みが走り、それぞれが過半数の応答を集めようとすると、両方の過半数に入るノードが必ず居ます。そのノードが、同じバージョンを元に作った 2 つ目の書き込みを断れば、両方が成立する事はありません。重なりが保証するのは、その判断ができるノードが必ず居る事までで、断る規則は別に必要です。
...
N 台の複製に対して、書き込みに必要な応答数を W、読み取りに必要な応答数を R と書きます。W と R の選び方には、目的の違う条件が 2 つあります。`W + R > N` を満たすと、書き込みに参加したノードの集合と、読み取りが応答を集めた集合は、必ず 1 台以上重なります。重なったノードは、最後に成功した書き込みの値を持っています。ただし、集めた R 個の応答のうちどれが最新かを見分ける手段は別に必要です（後述）。

`W > N / 2` を満たすと、書き込み同士も、必ず 1 台以上のノードで重なります。そのノードが両方を受け付ければ、2 つのバージョンがどちらも残ります。例えば、書き込みが元にしたバージョンと手元のバージョンが一致する時だけ受け付ける規則（compare-and-set）を置けば、同じバージョンを元に作った書き込みのうち成立するのは最大 1 つです。
...
上記の図の Node 3 が重なりです。N は 5、W と R はどちらも 3 なので `W + R = 6` となり、N を 1 台分超えます。5 台から 3 台ずつをどう選んでも、超えた 1 台分はどちらの集合にも入ります。
...
複製が 3 台（N = 3）の場合に選べる W と R は、以下の通りです。
...
分断が解消すると、少数派は多数派から差分を受け取って追い付きます。追い付かせる処理はクォーラムの外にあり、ログを複製する仕組みや、読み取りの時に古い値を見つけて直す仕組みが担当します。W を過半数に取れば、少数派で成立した書き込みはありません。W が過半数に届かない配分では、少数派でも書き込みが成立します。

W を過半数に取り、成功を返す前にディスクに書き終えていても、成功を返した更新が分断の後も残るとは限りません。書き込みを受け付けるノード（リーダー）を選び直す時や追い付かせる時に、その更新を持たないノードの内容で上書きすれば失われます。Raft では、リーダーを選ぶ票を出せるのは自分のログが候補より新しくないノードだけで、前のリーダーが作った更新は過半数に複製されただけでは確定と扱いません。この 2 つの条件は [Leader and Followers](../leader-and-followers/) のノートで扱っています。
...
- 複製を持たない単一ノードのシステム
- 書き込みの待ち時間に上限があり、過半数のノードの応答が揃うまで待てない場合
