---
title: "Leader and Followers - 更新を 1 台へ集約する複製の型"
date: 2026-08-07
draft: false
series: ["技術ノート"]
...
上図の Follower は、更新を受け取っても処理せずに Leader へ案内する決まりで、Leader から届いた内容を自分の複製へ反映します。Leader が落ちた場合は、残った Follower の中から次の Leader を選び直します。Leader を務めるノードは入れ替わり、役割は固定されていません。

---

### なぜ Leader が必要なのか

全ノードが対等に更新を受け付ける構成では、同じデータへの更新が別々のノードへ同時に届きます。各ノードは自分が受け取った順に処理してから相手へ複製するので、到着の順序が入れ替わると、追加の併合規則を置かない限り、最終的に残る値がノードごとに食い違います。

2 台が独立に更新を受け付けた場合に何が起きるかを以下に示します。
...
上図で問題なのは、どちらのノードも自分の見た順序で正しく処理しており、間違った動作をしていない点です。順序を決める場所がどこにも無いため、正しく動いた結果として値が 2 通りに割れています。後から一方を選び直すには、どちらが後の更新なのかを判定する材料が別に要ります。
...
更新の順序を 1 つに決めるには、全ノードが共有できる 1 つの並びを作る仕組みが要ります。Leader and Followers は、その並びを決める役目を Leader へ集約します。更新は必ず Leader を通り、Leader がログへ並べた順序を Follower が複製します。順序を後から復元する仕組みを用意せずに済む点が、この構成の中心にある効果です。
...
Follower は Leader から定期的に届く信号を待ちます。信号が一定時間届かなければ、Leader が落ちたと判断して自分が Candidate になり、任期を 1 つ増やして他のノードへ投票を求めます。クラスタの全構成ノードのうち過半数から票を集めたノードが、その任期の Leader になります。数えるのは応答があったノードの中の過半数ではなく、構成に含まれる全ノードの過半数です。信号そのものは[ハートビート](../heartbeat/)で、Raft では中身が空の複製要求を流用しています。
...
上図で Candidate から Candidate へ戻る経路は、複数のノードが同時に Candidate になって票が割れた場合を表します。全ノードが同じ待ち時間を使うと、票の割れが繰り返されます。そのため、Raft は待ち時間をランダムな幅で散らし、選出を始めるたびに引き直します。Leader から Follower へ戻る経路は、自分より新しい任期を検知した時に通ります。分断から復帰した旧 Leader がここを通るので、「古い Leader を締め出す」で改めて扱います。

投票を受け取った側の処理は短く書けます。Leader が 2 台立つのを防いでいるのは、1 つの任期で 2 票を投じない事と、当選に過半数を要求する事の組み合わせです。過半数の集合はどの 2 つを取っても 1 台以上を共有するので、共有するノードが 1 票しか出さなければ、同じ任期に 2 台が過半数へ届く事はありません。
...
func (n *node) handleRequestVote(term int, candidateID string) bool {
	...
	if term > n.currentTerm {
		// 新しい任期を見たら、投票の記録を捨てて Follower へ戻る
		n.currentTerm = term
		n.votedFor = ""
		n.role = roleFollower
	}
	if n.votedFor != "" && n.votedFor != candidateID {
		return false // 同じ任期で 2 台へ投票しない
	}
...
上のコードは `currentTerm` と `votedFor` をメモリ上で書き換えているだけで、実装としては不足しています。Raft は、この 2 つとログを RPC（Remote Procedure Call、遠隔手続き呼び出し）へ応答する前に安定ストレージへ書く事を要求しています。投票した直後に再起動して `votedFor` を失うと、同じ任期でもう 1 台へ投票してしまい、Leader が 2 台立ちます。

省略したログの新しさの検査は、確定済みの更新を守るために効いています。Raft は、自分のログが Candidate より新しいノードに投票を拒否させます。確定済みのエントリは過半数が持っており、票を集めるにも過半数が要るので、その 2 つは必ず重なります。確定済みのエントリを持たないノードは、重なったノードに拒否されて Leader になれません。重なりが効く理屈は[クォーラム](../quorum/)と同じです。
...
上図で Leader が Client へ応答したのは、Follower 2 の応答が届く前です。全ノードの応答を待たないため、1 台が遅れていても更新は進みます。遅れた Follower は、後から届いた分を自分のログへ追記して追い付きます。
...
```go
// advanceCommitIndex は、過半数へ複製できた位置まで commitIndex を進めます。
...
分断が起きてから旧 Leader が締め出されるまでを以下に示します。
...
sequenceDiagram
    ...
    B->>C: 投票を要求（任期 6）
    C-->>B: 投票する
    Note over B,C: Node B が任期 6 の Leader へ
    Note over A,C: 分断が解消
    A->>B: 複製要求（任期 5）
    B-->>A: 拒否（現在の任期は 6）
    Note over A: 任期 6 を見て<br/>Follower へ戻る
```

上図の Node A は、拒否の応答に含まれる任期番号を見て、初めて自分が交代させられた事を知ります。番号の大小だけで新旧を判定しており、時計は参照していません。権利を渡すたびに大きくなる番号で古い操作を拒否する点は、[Lease](../lease/) の fencing token と同じ発想です。

分断されている間、Node A が更新を確定させる事はありません。確定には過半数の応答が要り、少数派は定義上その数に届かないためです。確定していない更新なので、Client へ完了を返してもいません。分断が解消すると、Node A のログは任期 6 の Leader のものと突き合わせられ、食い違う位置から後ろが削られます。削られるのは衝突した範囲だけで、新しい Leader のログと一致している部分はそのまま残ります。
...
func (n *node) handleAppendEntries(args appendEntriesArgs) appendEntriesReply {
	...
	n.role = roleFollower
	n.leaderID = args.LeaderID // Client を現在の Leader へ案内するために覚える
...
常に最新の値が要る読み取りは、Leader を経由させます。Raft は、そのために 2 つの手順を要求しています（論文 8 節）。1 つ目は、Leader が就任した直後に自分の任期の空エントリを確定させる事です。`advanceCommitIndex` は現在の任期のエントリしか確定させないため、空エントリが無いと、就任した Leader は前任者が確定させた位置を把握できません。
...
論文は 2 つ目の代替として、ハートビートを Lease のように扱う方法も挙げています。読み取りのたびに過半数へ問い合わせずに済む代わりに、読み取りの安全性だけがタイミングに依存し、ノード間の時計のずれに上限がある事を仮定します。任期の比較やログの複製そのものが時計に依存するわけではありません。

ここまでの厳密さが要らない読み取りであれば、Follower へ流して構いません。集計や一覧の表示のように、数秒古い値でも判断が変わらない用途が候補になります。

---

### 利点

- 更新の順序が Leader の受け付け順に定まり、順序を後から復元する仕組みが要らない
...
- 数百台のノードを 1 つの合意グループへ参加させる構成。1 つのグループは数台に絞るのが前提で、システム全体の台数が多い事自体は問題になりません。規模を広げたい場合は、データを分割して小さなグループを複数立てます
- 更新が稀で、1 台の停止による短い中断を許容できるシステム

CRDT（Conflict-free Replicated Data Type）とは、複数のノードが独立に更新しても、後から併合すれば必ず同じ結果へ収束するように作ったデータ型です。順序を決める代わりに、順序が違っても同じ結果になる演算だけを許す方針だと考えられます。扱えるデータの形が限られるため、任意の更新を順序付けたい用途の代わりにはなりません。
...
| 全体の更新順序 | Leader が作る | 原則として要らない | そのノードが作る |
| 衝突の扱い | Leader のログの順で決まる | データ型の併合規則で収束させる | 衝突しない |
| 分断中の更新 | 少数派では確定できない | 構成次第で両側から更新できる | 繋がる側だけ |
| 向いている更新 | 順序と一意性が要るもの | 併合できるもの | 冗長化が要らないもの |

真ん中の列を CRDT に絞ったのは、全ノードで更新を受け付けるという一点では性質が決まらないためです。並べた軸も、更新をどう受けてどう揃えるかという意味の話に限りました。障害耐性・書き込みの性能・どのノードがどこまでのデータを持つかは、複製の置き方や配送の仕組みで決まるので、CRDT というデータ型を選んだだけでは定まりません。

Leader and Followers を選ぶ判断は、更新の性能と順序のどちらを取るかに寄ります。順序を諦めれば書き込みの受け口を増やせるので、併合できるデータだけを扱うと決められる場面では、CRDT による複製の方が素直です。決められない場面では、更新を 1 台へ集約する方が扱いやすいと考えられます。
