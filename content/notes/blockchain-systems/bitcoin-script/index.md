...
Script にはループがありません。[Bitcoin Wiki](https://en.bitcoin.it/wiki/Script) は、Script を意図的に（intentionally）チューリング完全にしていない、ループの無い言語だと説明しています。命令は左から右へ 1 度ずつ実行されるだけなので、実行される命令の数はスクリプトに書かれた数を超えず、どんなスクリプトも必ず停止します。
