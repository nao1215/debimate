...
しかし、この方式の取引には、Alice の残高のうちどの部分を取るのかを名指しする識別子が無く、2 つを並べて見比べても、同じ物を取り合っているとは表れません。残高を追って初めて、両方は成立しないと分かります。
...
同じ outpoint を参照する 2 つの取引のうち、どちらを残すかは UTXO だけでは決まりません。決めるには取引の順序についての合意が別に必要で、その順序を作る仕組みがブロックのチェーンです（[Bitcoin Block](../bitcoin-block/)）。UTXO が引き受けるのは、競合を参照の重複として見える形にする部分です。

なお、口座ごとに残高を保存する実際の方式はアカウントモデルと呼ばれ、Ethereum が採用しています。上の素朴な方式に取引の通し番号などを加えた表し方で、[Ethereum Account Model](../ethereum-account-model/) で説明しています。
...
手数料の項目は、取引のどこにもありません。出力の金額の合計が入力の合計より少なければ、差額が手数料となり、取引を含むブロックの報酬に加わります（[白書](https://bitcoin.org/bitcoin.pdf)）。この報酬を受け取るのは、そのブロックを作ったマイナーです。
...
- どのコインが誰の条件で使えるかは取引の連なりから分かり、UTXO 集合はそこから組み立て直せる
