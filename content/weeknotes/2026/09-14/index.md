...
スッとコードを読むときのために Neovim 環境を作った。私は、組み込み時代は画面全部を Terminal で覆って開発するスタイルだった。バックエンドエンジニアになってから、Visual Studio Code を使うようになり、NeoVim を使う機会がなかった。コードリーディングには Neovim が便利なので、今のうちに環境を作っておいた。書くなら VS Code、読むなら Neovim の使い分け。VS Code は最近やや重く、機能が増えすぎた。

昔は Terminal 背景を透過させたり、色んなプラグインを試したりしていたが、必要な機能だけを設定した。設定弄りに熱中すると、時間が勿体ない。
