...
スッとコードを読むときのために NeoVim 環境を作った。私は、組み込み時代は画面全部を Terminal で覆って開発するスタイルだった。バックエンドエンジニアになってから、Visual Studio Code を使うようになり、NeoVim を使う機会がなかった。コードリーディングには NeoVim が便利なので、今のうちに環境を作っておいた。書くなら VS Code、読むなら NeoVim の使い分け。

昔は背景を透過させたり、色んなプラグインを試したりしていたが、必要な機能だけを設定した。メインは開発であって、設定弄りではない。設定を変えるのは楽しいけどキリがないからね。
