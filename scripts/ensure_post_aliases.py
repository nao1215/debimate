...
def desired_aliases(index_path: Path, unique_dates: set[str]) -> list[str]:
    ...
    if len(dirname) > 11 and dirname[10] == "-":
        slug = dirname[11:]
...
def update_front_matter(index_path: Path, unique_dates: set[str], check_only: bool) -> bool:
    ...
    merged_aliases = list(dict.fromkeys(existing_aliases + desired_aliases(index_path, unique_dates)))
...
def main() -> int:
    ...
    for index_path in post_files:
        if update_front_matter(index_path, unique_dates, check_only=args.check):
            changed += 1
