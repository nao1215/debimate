...
ベクトル（vector）は数を 1 列に並べたもの、行列（matrix）は数を 2 次元に並べたものです。機械学習では、データ 1 件を「特徴量ベクトル」、データ集合を「行列」、モデルの重みを「ベクトル」または「行列」で表すのが標準で、内積・行列積といった演算がほぼすべてのアルゴリズムの計算基盤になっています。

ここでは、(1) ベクトルの加算・スカラー倍、(2) 内積の代数と幾何の両面、(3) 行列の積と線形変換としての見方、(4) 機械学習で頻出する「データ行列 × 重みベクトル」のパターン、の 4 点を整理します。線形回帰・ロジスティック回帰・ニューラルネット・PCA・SVM、いずれも `X @ w` という形の行列ベクトル積が中核に座っています。

### ベクトル: 加算・スカラー倍

ベクトル `a = (a_1, a_2, ..., a_n)` と `b = (b_1, b_2, ..., b_n)` の加算は、要素ごとの和です。

`a + b = (a_1 + b_1, a_2 + b_2, ..., a_n + b_n)`

スカラー `k` 倍は全要素を `k` 倍します。

`k a = (k a_1, k a_2, ..., k a_n)`

幾何的には、加算は平行四辺形則に対応し、スカラー倍は同じ向きへの伸縮（`k < 0` なら向きの反転）を表します。
...
左の図では `a = (3, 1)`、`b = (1, 2)` の和が `a + b = (4, 3)` となり、`a` の終点から `b` を継ぎ足した位置に対角線として現れます。右の図ではスカラー倍が方向を保ったまま長さだけ変える操作で、負のスカラーで反対側に伸びます。

機械学習でベクトルが出てくる典型は次の 3 場面です。
...
ベクトル `a` と `b` の内積は、対応する要素の積を足し上げたスカラー値です。

`a · b = a_1 b_1 + a_2 b_2 + ... + a_n b_n = Σ_i a_i b_i`

代数的にはそれだけだが、幾何的には次の関係が成り立ちます。

`a · b = ||a|| ||b|| cos(θ)`

ここで `||a||` は `a` の長さ（ノルム）、`θ` は 2 ベクトル間の角度です。この式から内積の符号で 2 ベクトルの方向関係が読み取れます。
...
左は鋭角の例で `a · b = 14 > 0`、右は鈍角の例で `a · b = -6 < 0` となります。緑の矢印は `b` を `a` の方向に射影した成分で、内積はこの射影の長さに `||a||` を掛けたもの、と読めます。

機械学習では「線形和」がほぼすべて内積として書けます。
...
行列 `A`（`m × k` サイズ）と `B`（`k × n` サイズ）の積 `C = A B` は `m × n` の行列で、要素 `C_{ij}` は「`A` の `i` 行」と「`B` の `j` 列」の内積で定義されます。

`C_{ij} = Σ_l A_{il} B_{lj}`

形状の制約として、`A` の列数と `B` の行数が一致しなければ積は定義されません。`(m × k) (k × n) → (m × n)` の形を覚えておくと事故が減ります。

性質として重要なのは、結合則は成り立つ（`(AB)C = A(BC)`）が交換則は成り立たない（一般に `AB ≠ BA`）ことです。順序を変えると意味が変わる、というのは線形変換の合成として見ると当然で、「先に回転してから拡大」と「先に拡大してから回転」が違うのと同じ理屈です。

線形変換としての見方を 1 つ持っておくと行列の理解が深まります。`A` を「ベクトル `x` を `Ax` に写す関数」とみなすと、`A` は空間全体の伸縮・回転・剪断（せん断）を表す変換になります。
...
灰色の正方形が元の単位正方形 `[0,1]^2`、赤色がその上に `A x` を適用した変換後の図形です。スケーリング（左）は軸方向の伸縮、回転（中央）は形を保ったまま角度だけ変える操作、剪断（右）は片側に滑らせる変形となります。任意の線形変換は、伸縮・回転・剪断の合成として理解できる、というのが [固有値分解](../eigen-decomposition/) や特異値分解（SVD）が浮かび上がってくる素地です。

---

### 機械学習で頻出する形: X w

教師あり学習では、`n` 個のデータと `d` 個の特徴量を「データ行列 `X`（`n × d`）」、重みを「ベクトル `w`（`d × 1`）」で表し、全サンプルの予測を 1 行の式で書きます。

`y_pred = X w` （`(n × d) (d × 1) = n × 1`）

各行のサンプル `i` についての予測 `y_pred_i = x_i · w` を `n` 個まとめて行列ベクトル積で表現したものに過ぎないが、numpy / pytorch などのライブラリでは for ループを書かずに 1 行で済む（BLAS で並列化される）ため、計算速度が桁違いになります。
...
左から「データ行列 `X`（縦に並ぶ各行が 1 サンプル、横に並ぶ列が特徴量）」「重みベクトル `w`（特徴量ぶんの長さ）」「予測 `y_pred = X w`（サンプルぶんの長さ）」と並べた図です。`X` の各行と `w` の内積が `y_pred` の対応する要素となります。

この `X w` パターンは線形回帰だけのものではなく、機械学習の中核に何度も現れます。
...
実装上のコツとして、numpy では `X @ w` または `np.dot(X, w)` で行列ベクトル積を書きます。要素ごとの積（アダマール積）`X * w` と取り違えると static エラーで気づくとは限らないので、形状の確認は `X.shape` で都度行うのが安全です。

---

### ノルム（vector の長さ）

ベクトルの「長さ」を測る関数をノルム（norm）と呼びます。代表的なのは次の 3 つで、機械学習では使い分けが効いてきます。

- L2 ノルム（ユークリッド距離）: `||x||_2 = sqrt(Σ_i x_i^2)`。最も標準的な距離
- L1 ノルム（マンハッタン距離）: `||x||_1 = Σ_i |x_i|`。座標軸方向の移動の総和
- L∞ ノルム（最大ノルム）: `||x||_∞ = max_i |x_i|`。最大の絶対値成分

[正則化](../../ml/regularization/) では L2 を 2 乗した `||w||_2^2`（Ridge）と L1 ノルム `||w||_1`（Lasso）が罰則項として用いられ、L1 は重みを 0 に寄せる（疎な解を作る）性質、L2 は重み全体を均等に小さくする性質を持ちます。
