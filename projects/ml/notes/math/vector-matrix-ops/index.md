...
性質として重要なのは、結合則は成り立つ（`(AB)C = A(BC)`）が交換則は成り立たない（一般に `AB ≠ BA`）ことです。順序を変えると意味が変わる、というのは線形変換の合成として見ると当然で、「先に回転してから縦横で違う比率に伸ばす」と「先に伸ばしてから回転する」が違うのと同じ理屈です。
...
これは、各行のサンプル `i` についての予測 `y_pred_i = x_i · w` を `n` 個まとめて行列ベクトル積で表現したものです。numpy / pytorch などのライブラリでは for ループを書かずに 1 行で済み（BLAS（Basic Linear Algebra Subprograms）という行列演算ライブラリで並列化される）、計算速度が桁違いになります。
