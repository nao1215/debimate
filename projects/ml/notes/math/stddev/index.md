...
標準偏差（standard deviation, stddev）は、[分散](../variance/) の平方根として定義される散らばりの指標です。分散と数学的には等価だが、「単位が元データと同じ」という違いが実用上は決定的に重要となります。例えば身長（cm）のデータなら、分散は `cm²` という解釈しにくい単位になる一方、標準偏差は `cm` のまま扱えます。

- 母標準偏差: `sigma = sqrt(sigma2)`
- 標本標準偏差: `s = sqrt(s2)`

`sigma2` や `s2` は分散を表し、詳細は [分散](../variance/) を参照です。

### 単位が揃うことの意味

「分散があるのになぜ標準偏差を別に定義するのか」という疑問は、単位を考えると自然に解けます。身長データで `mu = 170 cm`、`sigma2 = 25 cm²` だったとき、「平均から `25 cm²` ずれている」と言われても直感が働きません。一方、`sigma = 5 cm` と言われれば「平均 ± 5 cm の範囲がだいたい典型的なばらつきだ」と即座にイメージできます。

これは数学的に同じ情報を持つ量でも、「データと同じ単位で表現できるか」が運用上の使いやすさを大きく左右する例と言えます。実務で出す数字（誤差、信頼区間、リスクなど）はほぼ全て標準偏差ベースで報告されます。

### 68-95-99.7 ルール（正規分布の経験則）

データが正規分布に近い場合、標準偏差を「典型的なばらつきの目盛り」として直感的に使えます。
...
この感覚は異常検知の閾値設定や統計検定の判断に直結します。「3σ を超える値は異常」というルールはこの 99.7% 区間に基づいています。ただし正規分布から外れる（裾の重い分布、歪んだ分布）と崩れるので、ヒストグラムや [KDE](../kde/) で分布形を確認するのが先決と考えられます。
...
以下は、[平均（算術平均）](../mean/)の周りに標準偏差の幅を示した例です。
...
入れ子の 3 色の帯がそれぞれ ±1σ・±2σ・±3σ の範囲を表しており、覆っている確率の割合がラベルに書かれています。「3σ ルール」（観測値が ±3σ を超えれば異常）はこの 99.7% という数字が根拠で、正規分布ならば 1000 個に 3 個程度しか出ない極端な値、という意味になります。

ただし正規分布から外れる（裾の重い分布、歪んだ分布）と数字が大きく変わります。例として t 分布（自由度 5）では ±3σ の中に入る確率は 97% 程度に下がり、log-normal のような右に裾の長い分布ではさらに崩れます。実際のデータに 3σ ルールを適用する前に、ヒストグラムや [KDE](../kde/) で形を確認するのが安全と考えられます。

### Z スコアで異なる単位の値を比較する

Z スコア `z = (x - μ) / σ` を使うと、平均・標準偏差が違うデータ同士を「平均から何 σ離れているか」という共通単位で比較できます。
...
左の生スコアでは「数学 85 点 > 英語 58 点」で数学の方が高く見えるが、右の Z スコア表示では英語の方が `+1.34σ` と数学 `+1.04σ` より相対的に高い位置にいります。これは英語の母集団のばらつきが小さい（σ=5）ため、同じ生スコアの差でも母集団内での偏差としては大きく見える、という構造です。偏差値（`z × 10 + 50`）はこの考え方を 1 次変換しただけのもので、Z スコアは「異なる母集団の相対位置を統一する」道具となります。

機械学習の [標準化](../../ml/standardization/) も全く同じ操作で、特徴量を Z スコアに変換することで、距離ベース（kNN、k-means）や正則化を伴うモデル（Ridge、Lasso、ロジスティック回帰）が安定して動くようになります。

---

### Z スコアと標準化

標準偏差を使ってデータを「無次元化」する操作が Z スコア（Z-score）です。

```text
z = (x - mu) / sigma
```

これは「平均から何 σ だけ離れているか」を表す数で、単位を持ちません。Z スコアの利点は次の通りです。
...
Z スコアは「標準偏差を 1 単位の物差しにする」発想で、これによって異なる量の比較が可能になると言えます。

### 数学での使いどころ

数学・統計では標準偏差は以下で使われます。
