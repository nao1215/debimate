...
平均（算術平均, mean）は、データの「中心」を表す代表値の中で最も基本的な指標です。全ての値を足して個数で割るというシンプルな計算式ながら、確率論の期待値・最小二乗推定・大数の法則・中心極限定理の定義に直接現れます。後から学ぶ [分散](../variance/) ・ [標準偏差](../stddev/) ・ [相関係数](../correlation/) も内部で平均を使っており、後続の統計量を読むときの基準点として最初に押さえる量となります。

- 標本平均: `x_bar = (x1 + x2 + ... + xn) / n`
- 母平均: `mu = E[X]`

ここで `X` は「確率変数」（値がランダムに決まると考える対象）で、`E[X]` はその期待値（平均値）を表します。`mu` は母平均、`x_bar` は実際に手元のデータから計算する標本平均を意味します。

平均は直感的で計算も軽いが、外れ値に引っ張られやすいという弱点を持ちます。[中央値](../median/) との対比でいうと、平均は「値の重心」を見る指標（全ての値を平等に扱う）、中央値は「順位の中心」を見る指標（順位だけ見て値の大小は無視）という違いがあります。右に長い分布（給与・取引金額など）では平均が右側に引きずられるため、典型値を見るなら中央値の方が素直という場面が多くあります。

### 平均の数学的特徴付け

平均には「データを 1 つの代表値で要約するときの最適解」という数学的な性質があります。具体的には、

```text
mu = argmin_c  E[(X - c)^2]
```

これは「平均は L2 損失（二乗誤差）を最小にする点」という言い方ができます。対応する関係として、[中央値](../median/) は L1 損失（絶対誤差）を最小にする点です。この対比は機械学習の損失関数選びにも直結する: MSE（Mean Squared Error）損失で回帰すると平均的な振る舞いを当てに行き、MAE（Mean Absolute Error）損失で回帰すると中央値的な振る舞いを当てに行きます。

幾何学的には、平均は「データ点群の重心」と等しいです。各点が等しい質量を持つと考えたときの物理的な重心が平均、というイメージで、これが「外れ値が遠くにあると重心が引きずられる」直感を支えています。

平均と中央値の使い分けは、外れ値の有無と「何を最小化したいか」の 2 軸で整理できます。
...
以下は、平均と[中央値](../median/)の違いを可視化する簡単な例です。右に長い分布では平均が右側へ引っ張られます。
...
平均の「弱点」として頻繁に指摘されるのが、外れ値への感度の高さです。50 個の値（平均 50 周辺）に「500」という値を 1 つだけ追加すると、平均は 9 ほどジャンプするのに対し、中央値はほぼ動きません。
...
これは「平均は値の重心、中央値は順位の中心」という違いから直接来る性質で、データに極端な値が混ざる可能性がある場面（収入・取引額・反応時間など）では平均をそのまま代表値として読むと判断を誤る、と考えられます。

### L2 と L1 の最小化点としての平均・中央値

冒頭で示した `mu = argmin_c E[(X - c)^2]` の関係を実際に損失関数のグラフで確認します。
...
左の青い曲線は二乗誤差 `MSE(c) = mean((x - c)^2)` を `c` の関数として描いたもので、極小点が標本平均と一致します。右の絶対誤差 `MAE(c) = mean(|x - c|)` は極小点が標本中央値と一致します。回帰問題で MSE 損失を使うと平均的な振る舞いを学習し、MAE 損失を使うと中央値的な振る舞いを学習する、というのはこの関係から直接導かれます。

---

### 数学での使いどころ

数学・統計では平均は以下で使われ、ほぼ全ての基礎指標が平均を内部で使う構造になっています。
...
数学的には、平均は「値の重心」を表すと解釈できます。物理での重心、確率での期待値、最適化での L2 損失最小化、これらが全て同じ「平均」という量に集約される構造を持っています。

---

### 機械学習での使いどころ

機械学習では平均は前処理・最適化・評価のあらゆる場面で頻出します。
