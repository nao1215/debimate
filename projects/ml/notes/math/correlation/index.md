...
相関係数の土台となる量が共分散（covariance）である。共分散は [分散](../variance/) を 2 変数に拡張したものとして定義され、2 変数を同時に観測したときの [同時分布](../joint-marginal-conditional/) を要約する量と見ることもできる。
...
iris の 4 特徴量（萼片・花弁の長さ・幅）の相関行列で、赤が正の相関・青が負の相関・色の濃さが強さを表す。petal length と petal width の相関が `0.96` と極めて高く、これは「事実上同じ情報」と見ることができる。線形モデルや距離ベースのモデルでは [多重共線性](../../ml/regularization/) の原因になるため、片方を落とすか [PCA](../../ml/pca/) で集約する判断材料になる。
