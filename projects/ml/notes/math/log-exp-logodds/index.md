...
対数関数 `log(x)` と指数関数 `exp(x)` は互いに逆関数の関係にあり、機械学習では「積を和に変換する」「桁の違うスケールを揃える」「確率を log-odds に変換する」といった用途で繰り返し登場します。特に分類器の出力を「線形和としてモデル化する」発想は、`p / (1 - p)` を対数化した log-odds（対数オッズ）で表す形で実装されており、[ロジスティック回帰](../../ml/logistic-regression/) の中核を成します。

ここでは、機械学習で出てくる場面に絞って対数・指数の性質を整理します。具体的には、(1) log と exp の対応関係、(2) 「積を和に」変える代数的性質、(3) シグモイドと logit による確率↔log-odds の往復、(4) 浮動小数点の数値安定性を保つための log 空間計算、の 4 点です。
...
の関係が成り立ちます。`a = e ≈ 2.718` の場合を「自然対数」と呼び、単に `log(x)` あるいは `ln(x)` と書きます。機械学習・統計の文脈で「log」と書いてあれば原則として自然対数で、エントロピーなど情報理論の文脈で `log_2`、対数尺度の可視化で `log_10` が使われる程度の区別となります。
...
左の図は底だけ違う 3 種類の対数曲線で、いずれも `x = 1` で 0 を通り、形は同じです（互いに定数倍の関係）。右の図は `exp(x)` と `log(x)` を同じ座標に描いたもので、直線 `y = x` に対して鏡像になっています。これが「互いに逆関数」の幾何的な意味で、`exp(log(x)) = x` および `log(exp(x)) = x` が自然に成り立ちます。

定義域・値域の対応関係も重要で、`log` は正の数 `(0, ∞)` から実数全体 `(-∞, ∞)` へ、`exp` は実数全体から正の数 `(0, ∞)` へ、それぞれ全単射に写ります。負の数や 0 の対数は実数では定義されない点に注意が必要です。
...
左の線形スケールでは「5 倍」「10 倍」「50 倍」が傾きの違いとして発散していくのに対し、右の log スケールでは 3 本が同じ形を保ったまま縦方向に平行にずれています。これは `log(k x) = log(k) + log(x)` の式そのものを視覚化したもので、「倍率」が「平行移動」に変わる構造が見えます。

この性質は次の 3 つの場面で効いてきます。
...
確率 `p ∈ (0, 1)` のオッズ（odds）を `p / (1 - p)` で定義します。`p = 0.5` でオッズは 1、`p = 0.9` でオッズは 9、`p = 0.99` でオッズは 99 となります。オッズに log を被せた `log(p / (1 - p))` を log-odds または logit と呼びます。

`logit(p) = log(p / (1 - p))`

逆向きに、log-odds `z` から確率 `p` を取り戻す関数がシグモイド（sigmoid, logistic function）です。

`sigmoid(z) = 1 / (1 + exp(-z))`

両者は互いに逆関数の関係にあり、合わせると「`logit` で `(0, 1)` を `(-∞, ∞)` に伸ばし、`sigmoid` で `(-∞, ∞)` を `(0, 1)` に押し戻す」変換となります。
...
左のシグモイドは `z = 0` で `p = 0.5` を通り、`z` が大きくなると `p` は 1 に飽和、`z` が小さくなると 0 に飽和する S 字形となります。右の logit は `p = 0.5` で 0 を通り、`p` が 0 や 1 に近づくと急激に発散します。同じ「確率を 0.1 から 0.2 に動かす」操作と「0.8 から 0.9 に動かす」操作は、log-odds で見るとそれぞれ約 +0.81、+1.20 という別の値の動きであり、`p = 0.5` 付近の方が動かしやすい性質が読み取れます。

ロジスティック回帰がやっていることは、特徴量の線形和 `z = w · x + b` を「log-odds」と見なし、シグモイドで確率に変換する、という枠組みです。確率を直接線形和でモデル化すると `(0, 1)` の制約があるため扱いにくいが、log-odds なら値域が実数全体になるので線形和と素直につながります。詳細は [ロジスティック回帰](../../ml/logistic-regression/) のノートで扱います。

---

### log 空間で計算する: 数値安定性

確率は 0〜1 の間の値で、これを多数掛け合わせるとあっという間に浮動小数点の下限を下回り 0 に潰れます。例として、各サンプルの尤度を 0.99 として 800 個掛け合わせると、`0.99^800 ≈ 3.2 × 10^-4` と見かけ上は問題ないが、`0.5^1500` のような場面では `float64` の最小正規化数 `≈ 2.2 × 10^-308` を下回り、結果が 0 になってしまいます（underflow）。

log を被せると、積 `Π_i p_i` は和 `Σ_i log(p_i)` に変わります。`log(p)` は `p` が 0 に近いほど大きな負の値になるが、和を取る限り和の絶対値はサンプル数 `n` に比例する程度で、`float64` で安全に扱える範囲に収まります。
...
左の図では、確率の累積積はサンプル数を増やすと急激に小さくなり、視覚化のために log スケールを使っています。これを実コードで直接 `np.prod(...)` すると最終的には `0.0` に潰れます。一方で右の図では log 空間での累積和は線形に下がるだけで、何百万サンプルになっても float の表現範囲に収まります。

この理由から、最尤推定・EM アルゴリズム・隠れマルコフモデル・ナイーブベイズ・分類器の確率出力の交差エントロピー、いずれも「log を取ってから足し算」で実装されています。`scipy.special.logsumexp` や `torch.logsumexp` のように、`log(Σ exp(x_i))` を数値安定に計算する関数も標準で用意されており、softmax と組み合わせて頻繁に使われます。
