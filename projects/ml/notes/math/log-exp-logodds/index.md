...
左のシグモイドは `z = 0` で `p = 0.5` を通り、`z` が大きくなると `p` は 1 に飽和、`z` が小さくなると 0 に飽和する S 字形となります。右の logit は `p = 0.5` で 0 を通り、`p` が 0 や 1 に近づくと急激に発散します。同じ「確率を 0.1 動かす」操作でも、`0.4` から `0.5` への変化は log-odds で約 +0.41、`0.8` から `0.9` への変化は約 +0.81 であり、`p = 0.5` 付近の方が動かしやすい性質が読み取れます。

ロジスティック回帰がやっていることは、特徴量の線形和 `z = w · x + b` を「log-odds」と見なし、シグモイドで確率に変換する、という枠組みです。確率を直接線形和でモデル化すると `(0, 1)` の制約があるため扱いにくくなります。log-odds なら値域が実数全体になるので、線形和と素直につながります。詳細は [ロジスティック回帰](../../ml/logistic-regression/) のノートで扱います。

---

### log 空間で計算する: 数値安定性

確率は 0〜1 の間の値で、これを多数掛け合わせるとあっという間に浮動小数点の下限を下回り 0 に潰れます。例として、各サンプルの尤度を 0.99 として 800 個掛け合わせると、`0.99^800 ≈ 3.2 × 10^-4` と見かけ上は問題ありません。しかし、`0.5^1500` のような場面では `float64` の最小正規化数 `≈ 2.2 × 10^-308` を下回り、結果が 0 になってしまいます（underflow）。

log を被せると、積 `Π_i p_i` は和 `Σ_i log(p_i)` に変わります。`log(p)` は `p` が 0 に近いほど大きな負の値になります。ただし、和を取る限り和の絶対値はサンプル数 `n` に比例する程度で、`float64` で安全に扱える範囲に収まります。
...
この理由から、最尤推定・EM アルゴリズム（Expectation-Maximization、期待値最大化法）・隠れマルコフモデル・ナイーブベイズ・分類器の確率出力の交差エントロピー、いずれも「log を取ってから足し算」で実装されています。`scipy.special.logsumexp` や `torch.logsumexp` のように、`log(Σ exp(x_i))` を数値安定に計算する関数も標準で用意されており、softmax と組み合わせて頻繁に使われます。
...
- 単位の意味が変わる: log で変換した特徴量は元の単位を持たない（`log(売上)` の係数 `0.3` は「売上が 1% 増えると目的変数が約 0.003 動く」という意味で、目的変数も log を取った場合に限り「1% 増えると 0.3% 動く」という弾力性の解釈になる）
