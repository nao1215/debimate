...
[データドリフト](../../mlops/data-drift/) で 4 種類の drift を切り分けたり、ベイズの定理を扱ったり、分類器の出力を確率として解釈したりと、3 つの分布の区別ができないと先に進みづらい場面が多いです。この 3 つは独立した別物というより、同じ同時分布から取り出し方が違うだけ、と考えると整理しやすいです。
...
関係式を 1 行で書くと次の通りです。これは「乗法定理」とも呼ばれ、ベイズの定理の出発点になります。
...
具体例として「雲量 `x` ∈ {少, 中, 多}」と「雨 `y` ∈ {降る, 降らない}」を考えます。観測データから次のような同時分布が得られたとします。
...
中央のヒートマップ 6 マスが同時分布 `P(x, y)` で、すべての値を足すと 1.0 になります。上端の緑のバーは「各列を縦に足し上げて得た `P(y)`」、右端の赤のバーは「各行を横に足し上げて得た `P(x)`」です。図の端っこ（margin）にできるので marginal（周辺）と呼ばれる、と覚えると名前が腑に落ちると言えます。

---

### 周辺分布（marginal distribution）

複数の変数を持つ同時分布から、一部の変数だけに注目して取り出した分布を周辺分布と呼びます。求め方は、注目しない変数について「足し合わせる」（連続なら「積分する」）だけです。
...
機械学習の文脈では `P(y)` を「クラス比率」「事前分布」「prior」と呼ぶことが多いです。例えば、不正取引検知では `P(y=1) = 0.01`（陽性 1%）が `P(y)` にあたります。ラベル分布が時間で変わる現象は [label shift / prior shift](../../mlops/data-drift/) と呼ばれ、`P(y)` が動く drift の一種です。

---

### 条件付き分布（conditional distribution）

片方の変数をある値に固定したときの、もう片方の分布です。同時分布の「特定の行」または「特定の列」を取り出して、それが確率分布として 1 に正規化されるよう全体を割る操作で得られます。
...
「雲量が多いとき雨が降る確率」は 75% で、「雲量が少ないとき雨が降る確率」は 7% である、と読めます。条件付き分布は機械学習で最も多用される量と言えます。分類器の `predict_proba` が出すスコアは、まさに `P(y=1 | x)`（入力 `x` が与えられたとき、ラベルが 1 である確率）の推定値です。

---

### 連続の例: 2 次元正規分布

連続変数の場合も考え方は同じで、「足し合わせる」が「積分する」に変わるだけです。相関 0.7 の二次元正規分布から 1000 点をサンプリングし、散布図と周辺ヒストグラムを描きます。
...
中央の散布図が同時分布 `P(x, y)` のサンプル可視化で、上端と右端のヒストグラムがそれぞれ `P(x)` と `P(y)` の周辺分布です。連続の場合、条件付き分布 `P(y|x=a)` は「`x=a` 付近の縦の細い帯に入る点だけ取り出した分布」に対応します。`x` と `y` に [相関](../correlation/) があるため、`x` が大きい領域では `y` も大きめ、という関係が見えます。

---

### 独立 vs 従属を視覚的に区別する

独立性（independence）の数学的定義は `P(x, y) = P(x) × P(y)` で、これが成り立つときに限り「`X` を知っても `Y` の予測には何も寄与しない」となります。同時分布の散布図を見れば、独立か従属かは形からほぼ判定できます。
...
左の独立な散布図は円形に近く、どの `x` の帯を切ってもその中の `y` の分布形はほぼ同じになります（= `P(y|x)` が `x` に依存しない = `P(y|x) = P(y)`）。右の従属な散布図は楕円形に細長く伸び、`x` の小さい領域では `y` も小さく、`x` の大きい領域では `y` も大きい、という関係が見えます。これが「`X` を知ると `Y` の分布が動く」、つまり従属性を視覚的に示しています。

注意点として、Pearson 相関 `r ≈ 0` は「線形関係が無い」ことしか示さず、独立性とは別物です。`y = x^2` のような非線形依存があると `r ≈ 0` でも完全に従属であり、独立性は相互情報量（mutual information）や [相関係数](../correlation/) のノートで触れた他の指標で確認する必要があります。

---

### 条件付き分布をスライスとして見る

散布図の「縦の帯」として、連続変数での条件付き分布 `P(y|x=c)` を直接取り出してヒストグラムにします。
...
左の散布図で 3 つの縦の色帯がそれぞれ `x ≈ -1.5`, `0`, `+1.5` 付近の点を取り出した範囲で、その中の `y` 値だけを集めてヒストグラム化したのが右の図です。条件 `x` が動くと条件付き分布 `P(y|x)` の中心が動く（`x` の増加とともに右に移動）のが見て取れます。

回帰モデルが学習している関数 `f(x) = E[y|x]` は、まさにこの「`x` を固定したときの `y` の期待値」を `x` の関数として推定するものに他なりません。`predict_proba` も同じく `P(y=1|x)` の推定であり、条件付き分布の視点はそのまま予測モデルの設計思想と一致します。

---

### ベイズの定理との関係

3 つの分布をつなぐ式から、ベイズの定理が直接導出できます。
...
これがベイズの定理です。読み方は「事後分布 `P(y|x)` は、尤度 `P(x|y)` と事前分布 `P(y)` の積を証拠 `P(x)` で正規化したもの」となります。同時分布から始めると、ベイズの定理は特別な公式ではなく、3 つの分布の関係を変形しただけのものと考えられます。

機械学習では `P(y|x)`（入力から出力を予測）を直接学習するモデル（識別モデル: ロジスティック回帰、決定木、ニューラルネット）と、`P(x|y)` と `P(y)` を別々に学習してベイズの定理で `P(y|x)` を作るモデル（生成モデル: Naive Bayes、LDA、GAN、Diffusion）の 2 系統があり、3 つの分布のどれを直接モデル化するかで設計思想が分かれます。
...
3 つの分布の区別は、教科書の数式を読むときと、自分のモデルの出力を解釈するときの両方で効いてきます。
...
- 「`P(y|x)` と `P(x|y)` は同じ」ではない: 条件と被条件を入れ替えると意味も値も変わる。雨の例では `P(雨|雲量=多) = 0.30 / 0.40 = 0.75` と `P(雲量=多|雨) = 0.30 / 0.40 = 0.75` がたまたま同じ値になるものの、問いも意味も違う。混同するとベイズの定理の意味も失われる
