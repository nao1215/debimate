...
中央のヒートマップ 6 マスが同時分布 `P(x, y)` で、すべての値を足すと 1.0 になります。上端の緑のバーは「行方向に足し上げて得た `P(y)`」、右端の赤のバーは「列方向に足し上げて得た `P(x)`」です。図の端っこ（margin）にできるので marginal（周辺）と呼ばれる、と覚えると名前が腑に落ちると言えます。
...
機械学習の文脈では `P(y)` を「クラス比率」「事前分布」「prior」と呼ぶことが多いです。不正取引検知で `P(y=1) = 0.01`（陽性 1%）のような数字がそれにあたります。ラベル分布が時間で変わる現象は [label shift / prior shift](../../mlops/data-drift/) と呼ばれ、`P(y)` が動く drift の一種です。
