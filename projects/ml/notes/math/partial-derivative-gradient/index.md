...
機械学習では損失関数 `L(w_1, w_2, ..., w_n)` をモデルのパラメータで偏微分し、勾配の逆向きにパラメータを少しずつ動かして損失を下げる、という枠組みが繰り返し出てきます。線形回帰の正規方程式は勾配を 0 と置いて解いたもので、ロジスティック回帰の最尤推定やニューラルネットの誤差逆伝播は勾配の逆向きに進んで解を探します。いずれも損失の勾配を求めるところから始まります。

### 1 変数の微分から始める

1 変数関数 `f(x)` の微分 `f'(x)` は、`x` を少しだけ動かしたときに `f` が動く比率を表します。グラフ上では、点 `(x, f(x))` での接線の傾きと同じものとなります。
...
`f(x) = x^2` の `x=1` における微分は `f'(1) = 2` で、図の赤い破線の傾きと一致します。微分は局所的な傾きであり、グラフが平らな点（`f'(x) = 0`）は極大・極小・鞍点の候補となります。最適化はこの「平らな点」を探す作業でもあります。

---

### 偏微分: 1 変数だけ動かす

多変数関数 `f(x, y) = x^2 + 2 y^2` を考えます。`x` についての偏微分 `∂f/∂x` は、`y` を固定したまま `x` だけ動かしたときの変化率です。形式的には `y` を定数とみなして 1 変数の微分を取ります。

- `∂f/∂x = 2x`（`y` は定数扱い、`2 y^2` の項は微分で消える）
- `∂f/∂y = 4y`（`x` は定数扱い、`x^2` の項は微分で消える）

幾何的には、`y = y_0` という平面でサーフェスを切った断面の傾きが `∂f/∂x` に、`x = x_0` で切った断面の傾きが `∂f/∂y` に対応します。
...
赤い曲線が `y = y_0` 平面での断面で、その曲線の傾きが `(x_0, y_0)` における `∂f/∂x` の値となります。緑の曲線は `x = x_0` 平面での断面で、傾きが `∂f/∂y` を表します。1 つの点に対して、見る方向ごとに別々の傾きが定まる、という点が多変数関数の特徴です。

---

### 勾配ベクトル: 偏微分を並べる

偏微分を全変数ぶん集めてベクトルにしたものを勾配と呼びます。記号は `∇f`（ナブラ f）。

`∇f(x, y) = (∂f/∂x, ∂f/∂y)`

`f(x, y) = x^2 + 2 y^2` の場合は `∇f = (2x, 4y)` となります。

勾配には次の 2 つの幾何的性質があります。

- 向き: 勾配ベクトルは「`f` が最も急に増える方向」を指す
- 大きさ: 勾配ベクトルの長さ `|∇f|` は、その方向への変化率の大きさを表す

そのため、最小化したい関数では `-∇f`（負の勾配、descent direction）の向きに動けば、最も急に値が下がります。これが最急降下法（gradient descent）の出発点となります。詳細は [最急降下法・SGD](../gradient-descent-sgd/) のノートに譲ります。

等高線図上で見ると、勾配ベクトルは等高線に常に垂直に立ちます。等高線は「同じ高さの点をつないだ線」なので、その線に沿って動いても高さは変わりません。最も大きく高さが変わる向きはその直交方向、つまり等高線の法線方向となります。
...
赤い矢印が各点の勾配ベクトル（正規化済み）で、青い等高線に直交しています。原点が最小値で、勾配の長さは原点から離れるほど大きくなります。`y` 方向は係数 4、`x` 方向は係数 2 なので、矢印は `y` 方向に強めに傾く点も見えます。

---

### 機械学習に直結する例: MSE 損失の勾配

線形モデル `y_pred = w * x + b` を MSE（平均二乗誤差, mean squared error）で訓練する状況を考えます。
...
となります。勾配は `(w, b)` 平面上のベクトルで、その逆向きに `(w, b)` を更新していけば損失が下がります。学習率 `η` を掛けて
...
色の濃淡が損失の高さで、星が真の最適解 `(w=2, b=0)` の位置です。各点の赤い矢印は `-∇L`（負の勾配）の向きで、いずれも星の方向を指しています。最急降下法はこの矢印に沿ってパラメータを動かすことで、損失面を「下る」アルゴリズムだと考えられます。

---

### 連鎖律（chain rule）

合成関数 `f(g(x))` の微分は、`f'(g(x)) × g'(x)` という積で書けます。これが連鎖律で、ニューラルネットの誤差逆伝播法はこの連鎖律を層をまたいで適用したものに他なりません。

多変数の連鎖律は、層 `L = f(z(w))`（`z` は中間出力、`w` は重み）に対して

`∂L/∂w = (∂L/∂z) × (∂z/∂w)`

のように偏微分の積で書けます。「外側の損失の勾配」と「内側の関数のヤコビ行列」を順に掛け合わせる構造で、深いネットワークでも 1 層ずつ局所的に勾配を計算できる仕組みになります。
...
- ベクトル場の物理的意味（重力・電場など）
- [固有値分解](../eigen-decomposition/) と関連する 2 次形式 `x^T A x` の勾配は、`A` が対称行列なら `2 A x`（一般の `A` では `(A + A^T) x`）

---

### 機械学習での使いどころ

機械学習で勾配が登場しない場面はほとんどありません。
...
- ハイパーパラメータ最適化: ベイズ最適化のような勾配を直接使わない手法もある。ただし、微分可能な領域なら勾配ベース最適化が高速

---

### 適さないケース / 限界

勾配ベース最適化は便利です。ただし、すべての関数に効くわけではありません。
