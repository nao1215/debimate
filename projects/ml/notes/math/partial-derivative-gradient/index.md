...
機械学習では損失関数 `L(w_1, w_2, ..., w_n)` をモデルのパラメータで偏微分し、勾配の逆向きにパラメータを少しずつ動かして損失を下げる、という枠組みが繰り返し出てきます。線形回帰の正規方程式から始まり、ロジスティック回帰の最尤推定、ニューラルネットの誤差逆伝播まで、根は同じ「勾配の符号を反対側に進む」操作です。
...
勾配ベース最適化は便利だが、すべての関数に効くわけではありません。
