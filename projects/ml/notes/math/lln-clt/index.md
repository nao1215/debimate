...
統計的推測 ([仮説検定](../hypothesis-test/)) の理論的基盤、機械学習での bootstrap・分位回帰・confidence interval、A/B テストの効果検証など、ML / 統計の至るところで前提として使われます。すでに [期待値](../expectation/) のノートで「サンプル平均が期待値に収束する」現象を可視化した（大数の法則）が、ここではそれと CLT を統一的に整理します。

### 大数の法則: 平均は期待値に収束する

独立同分布（i.i.d.）の確率変数 `X_1, ..., X_n` について、サンプル平均 `X̄_n = (X_1 + ... + X_n) / n` は `n → ∞` で真の期待値 `μ = E[X]` に収束します。

`X̄_n →ᵖ μ` （確率収束）

「直感」: ランダムなノイズは平均を取れば打ち消しあって、真の中心だけが残ります。
...
3 通りの試行（青・赤・緑）は、それぞれ初期に揺れています。サンプル数が増えると、すべて黒い破線（期待値 3.5）に収束します。横軸 log スケールで `n = 100` あたりからほぼ落ち着き、`n = 1000` ではほぼ動かなくなります。
...
ML の実用では区別を意識することは少なく、「サンプル数を増やせば推定値は真値に近づく」という保証として使う場面が多いです。

---

### 中心極限定理: サンプル平均は正規分布に近づく

LLN は「サンプル平均が `μ` に集中する」と言うだけで、`μ` の周りでどう散らばるかは何も言いません。これを補完するのが CLTです。

独立同分布（i.i.d.）の確率変数 `X_1, ..., X_n` の平均 `X̄_n` について、`n → ∞` で標準化した量が標準正規分布に分布収束します。

`(X̄_n - μ) / (σ / √n)  →ᵈ  N(0, 1)`

ここで `μ = E[X]`、`σ² = Var(X)`です。等価な書き方として、`X̄_n` 自体が `N(μ, σ²/n)` に近づく、と書けます。

中心極限定理の要点は「元の `X` の分布によらない」という普遍性です。一様分布、指数分布、ベルヌーイ分布、対数正規分布、いずれを取っても、その平均は正規分布に収束します。
...
上段の元分布は全部形が違います。一様（矩形）、指数（右に裾の重い）、ベルヌーイ（2 点離散）、対数正規（強く右に歪む）。にもかかわらず下段の「30 サンプル平均」の分布はすべて釣鐘型で、赤い理論曲線 `N(μ, σ²/n)` にほぼ一致します。

これが「正規分布が至るところに現れる」最大の理由です。複数の独立な要因の加算的な効果は、個々の要因が何であれ正規分布に近づきます。身長・テストスコア・測定誤差・株価変動などが正規分布で近似できるのは、複数の独立要因の和とみなせるからと考えられます。

---

### n が大きくなるほど集中 + 正規化

CLT と LLN を 1 つの実験で見ます。対数正規分布を例に、`n` を変えてサンプル平均の分布を描きます。
...
`n = 1` は元の対数正規分布そのものです。強く右に歪んだ形です。`n = 5` で歪みが緩んできて、`n = 30` でほぼ正規型、`n = 200` で真値 `E[X] ≈ 1.65`（赤い破線）の周りに鋭く集中します。

spread は `n = 1` の 2.14 から `n = 200` の 0.15 へ、おおむね `1/√n` の速度で縮みます。これが「サンプル数を 4 倍にすると精度が 2 倍になる」根拠で、次の標準誤差の議論に繋がります。

---

### 標準誤差は 1/√n で縮む

サンプル平均 `X̄_n` の標準偏差を標準誤差（standard error, SE）と呼びます。

`SE(X̄_n) = σ / √n`

ここで `σ` は元の分布の標準偏差、`n` はサンプル数です。実データでは `σ` を `sample std` で近似します。
...
両対数グラフで、青い実測点（標準誤差）と赤い理論線 `σ/√n` がきれいに一致します。傾きは -0.5、すなわち `n` を 4 倍にすると SE が半分、`n` を 100 倍にすると SE が 10 分の 1です。
...
「データを増やせば精度は上がる。ただし、対費用効果は急速に逓減する」というのが、現実の実験計画・A/B テスト設計の基本となります。

---

### 95% 信頼区間と CLT

CLT を直接使うと、サンプル平均の信頼区間が次のように書けます。

`CI_95% = X̄_n ± 1.96 × (σ / √n)`

`1.96` は標準正規分布の上側 2.5% 点です。「同じ実験を多数回繰り返したとき、95% の区間が真値 `μ` を含む」というのが頻度主義的な解釈で、[仮説検定](../hypothesis-test/) のノートで詳しく扱っています。

CLT がなければこの計算は成立しません。元の分布が何であろうと、サンプル平均が正規分布に近づくので、`±1.96σ/√n` という単純な計算で信頼区間が出ます。

---

### 限界と例外

CLT は強力です。ただし、無条件に使えるわけではありません。
...
経験則として「`n ≥ 30` あれば CLT は近似的に成り立つ」とよく言われます。ただし、これは元分布が「中程度の歪み」までの場合の目安です。強く歪んだ分布や離散分布では、より大きな `n` が必要となります。実データで CLT 近似が成り立つかは、Q-Q プロット（quantile-quantile plot）で正規分布との直線性を視覚的に確認するのが安全です。
...
- LLM の MCQ（Multiple Choice Question、多肢選択式問題）評価: 多数サンプルの確率平均が真の選好確率に収束
...
- 分散が無限の分布（コーシー、Pareto with α≤2）: CLT が成り立たない。期待値まで存在しない場合（コーシー、Pareto with α≤1）は LLN も成り立たず、サンプル平均が収束しない
- 相関のあるサンプル: 時系列、空間相関、繰り返し測定。有効サンプル数 `n_eff` で計算するか、相関を考慮した手法へ
- 「サンプル平均が正規分布だから個々のサンプルも正規」と誤解: CLT は平均についての話で、元分布の形は変わらない
- 重みなしサンプル平均でしか CLT を使わない: 重み付き平均にも一般化された CLT がある。ただし、有効サンプル数の計算が複雑になる
- 不均衡データを補正なしに評価: 少数クラスのサンプル数が少ないと、そのクラスの metric の SE が大きい。stratified bootstrap などで補正
