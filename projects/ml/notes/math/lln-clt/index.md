...
3 通りの試行（青・赤・緑）はそれぞれ初期に揺れるが、サンプル数が増えるとすべて黒い破線（期待値 3.5）に収束します。横軸 log スケールで `n = 100` あたりからほぼ落ち着き、`n = 1000` ではほぼ動かなくなります。
...
「データを増やせば精度が上がるが、対費用効果は急速に逓減する」というのが、現実の実験計画・A/B テスト設計の基本となります。
...
経験則として「`n ≥ 30` あれば CLT は近似的に成り立つ」とよく言われるが、これは元分布が「中程度の歪み」までの場合の目安です。強く歪んだ分布や離散分布では、より大きな `n` が必要となります。実データで CLT 近似が成り立つかは、Q-Q プロット（quantile-quantile plot）で正規分布との直線性を視覚的に確認するのが安全です。
