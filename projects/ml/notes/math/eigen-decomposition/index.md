...
を満たす `v ≠ 0` と `λ` をそれぞれ固有ベクトル・固有値と呼びます。一般のベクトルは `A` をかけると向きも長さも変わります。一方、固有ベクトルは長さだけが `λ` 倍に変わり、向きはそのままになります。

機械学習では [主成分分析（PCA）](../../ml/pca/) の中核に共分散行列の固有値分解が座っており、データの「最も伸びている方向」「次に伸びている方向」を順に固有ベクトルが捉えます。線形変換の見方（[ベクトルと行列の演算](../vector-matrix-ops/) 参照）と組み合わせると、固有値・固有ベクトルは「その変換の骨格」を取り出す道具と読めます。

### 固有ベクトル: 向きが変わらない方向

行列 `A = [[2, 1], [0, 3]]` を例に取ります。
...
![A をかけると大半は向きが変わる一方、固有ベクトルは方向を保つ](./eigen_action.svg)

左の図は単位円周上に並べた青いベクトル群と、強調された 2 本の赤い固有ベクトルです。右の図は同じベクトル群に `A` をかけた結果で、青い大半のベクトルは向きが変わっているのに対し、赤い固有ベクトル方向だけは伸び方向が保たれています。それぞれの固有値 `λ_1 = 3, λ_2 = 2` が、固有ベクトル方向への伸び率に対応しています。

固有値の符号と大きさには次の意味があります。
...
の形に分解できます。ここで `Q` は固有ベクトルを列に並べた行列、`Λ` は固有値を対角に並べた対角行列です。

対称行列の場合は `Q` が直交行列となり `Q^-1 = Q^T`、

`A = Q Λ Q^T`

と簡潔に書けます。共分散行列はもともと対称なので、機械学習ではこの形が標準と考えてよいです。

この分解の幾何的な意味は「`A` の作用は『固有ベクトル基底に座標変換 → 各軸を固有値倍にスケーリング → 元の座標系に戻す』の 3 段階の合成として書ける」ことです。
...
任意の対称行列は「直交回転 → 軸方向の伸縮 → 直交回転」の合成、というのが固有値分解の幾何的な核心です。対称行列に限らない一般の行列へ拡張したものが特異値分解（SVD, singular value decomposition）で、こちらは正方でなくても適用できる代わりに左右で別の直交行列を使う `A = U Σ V^T` の形になります。

---

### 共分散行列の固有値分解 = PCA

機械学習で固有値分解が最も使われるのは、データの共分散行列 `Σ = (1/n) X^T X`（中心化済み）の分解です。これがそのまま [PCA](../../ml/pca/) で、主成分は共分散行列の固有ベクトル、各主成分の分散の寄与は対応する固有値となります。
...
データ点群の散らばりの「主軸」が固有ベクトル方向と一致しています。第 1 主成分（赤、最も大きい固有値）はデータが最も伸びる方向を指し、第 2 主成分（緑、2 番目の固有値）はそれに直交する方向で残りの分散を最大に説明する方向を指します。矢印の長さを `√λ` に取ることで、固有値がそのまま分散の大きさを表す関係も可視化されています。

固有値分解はもうひとつ重要なものを与えます。固有値の配列（スペクトル）を見ると、各成分がデータのどれくらいの分散を説明するかが分かります。
...
左の棒グラフがスペクトル（各主成分の固有値）、右が「各成分が説明する分散の比率」と「累積寄与率」です。この例では上位 2 成分で全分散の約 87%、上位 3 成分で 94% を説明できることが読み取れます。「何次元に削減するか」を決めるとき、累積寄与率を見ながら 90%・95% のような目安で切る運用が標準です。
...
固有値分解 / 特異値分解は機械学習の数式の至る所に現れます。

- [PCA](../../ml/pca/): 共分散行列の固有値分解で次元削減
- 特異値分解（SVD）: 長方形行列の分解、潜在因子モデル、行列補完
- スペクトラルクラスタリング: グラフラプラシアンの小さい固有値に対応する固有ベクトルでクラスタリング
...
固有値分解は強力です。その一方で、適用条件と数値特性には注意が必要です。

- 一般の長方形行列には適用できない: 正方行列が前提。長方形なら特異値分解（SVD）を使う
- 対角化できない行列がある: 同じ固有値を複数持つ場合に、線形独立な固有ベクトルが足りなくなる事があり、その時はジョルダン標準形が必要になる。実用上はほぼ気にしなくてよい。数値計算では摂動で対角化可能になることが多い
