...
期待値は「[平均](../mean/)」と混同しやすいが、両者は別物です。サンプル平均（標本平均, `x̄ = Σ x_i / n`）は手元のデータから計算する量、期待値は確率分布（または真の母集団）から計算する量です。サンプルが多ければ平均は期待値に近づくが、これを保証するのが次の大数の法則です。
...
左の図は 3 通りの試行で、サンプル数を 1 〜 5000 と増やしたときの累積平均の推移です。最初は試行ごとに揺れるが、サンプル数が増えると 3 本とも黒い破線（期待値 3.5）に収束していきます。右の図は「`n` サンプルでの平均値」を 1000 回繰り返した分布で、`n = 10` では幅広く、`n = 100`、`n = 1000` と増やすにつれて期待値の周りに鋭く集中します。「`1/√n` のオーダーで収束する」という具体的な速度は中心極限定理が記述する内容で、機械学習でも標準誤差や信頼区間の計算に直結します。
...
対称な分布（正規分布など）では `E[X]` は中央値・最頻値と一致するが、非対称な分布では大きくずれます。代表的な分布の `E[X]` 公式は [代表的な確率分布](../probability-distributions/) のノートにまとめてあります。
...
左の正規分布では `E[X]` がピーク（最頻値）と一致するが、右の対数正規分布では「裾」の長い右側に期待値が引っ張られ、最頻値より大きい位置に来ます。期待値は外れ値の影響を強く受ける、というのが [平均と中央値](../median/) の議論と同じ構造です。実用ではロバスト性が必要なときに中央値や trimmed mean を併用します。
...
3 つのヒストグラムが `X`、`Y`、`X + Y` のサンプル分布で、それぞれの期待値の位置に赤い破線を引いてあります。右図の `X + Y` の平均と、`X` と `Y` の平均の和が一致しています（赤と黒のラインがほぼ重なる）。`X` と `Y` は相関を持つように作ってあるが、期待値の線形性は独立性を要求しないため、それでも等号が成り立ちます。
