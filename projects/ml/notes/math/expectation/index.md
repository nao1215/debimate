...
機械学習で出てくる損失関数の多くは、データ分布に対する期待値の形をしています。`L(θ) = E_{(x, y) ~ p}[ℓ(x, y, θ)]` のように書かれ、これを訓練データという有限サンプルの平均で近似する、というのが「経験リスク最小化」と呼ばれる枠組みです。バイアス・バリアンス、強化学習の報酬、KL ダイバージェンスなど、確率モデルが出てくる場面では必ず期待値の言葉で議論されます。

### 離散の期待値

サイコロを少し細工して、目が出る確率が下の表のように偏っているとします。
...
赤い破線が `E[X] = 4.25` の位置で、青い棒（確率分布）の「重心」に対応します。期待値そのものが取りうる値のひとつであるとは限りません（実際この例で 4.25 という目はサイコロには存在しない）。値域に含まれない値が期待値になるのは、確率変数の定義上ごく普通の現象です。

期待値は「[平均](../mean/)」と混同しやすい概念です。ただし、両者は別物です。サンプル平均（標本平均, `x̄ = Σ x_i / n`）は手元のデータから計算する量、期待値は確率分布（または真の母集団）から計算する量です。サンプルが多ければ平均は期待値に近づきます。これを保証するのが次の大数の法則です。

---

### 大数の法則: サンプル平均が期待値に近づく

期待値の存在意義は、「サンプル平均が増えるほど期待値に収束する」という性質に支えられています。これを大数の法則（law of large numbers）と呼びます。

公平なサイコロを振り続け、ロール数の増加に伴うサンプル平均の動きを描きます。期待値は `E[X] = (1+2+3+4+5+6)/6 = 3.5` です。
...
左の図は 3 通りの試行で、サンプル数を 1 〜 5000 と増やしたときの累積平均の推移です。最初は試行ごとに揺れています。サンプル数が増えると、3 本とも黒い破線（期待値 3.5）に収束していきます。右の図は「`n` サンプルでの平均値」を 1000 回繰り返した分布で、`n = 10` では幅広く、`n = 100`、`n = 1000` と増やすにつれて期待値の周りに鋭く集中します。「`1/√n` のオーダーで収束する」という具体的な速度は中心極限定理が記述する内容で、機械学習でも標準誤差や信頼区間の計算に直結します。
...
対称な分布（正規分布など）では `E[X]` は中央値・最頻値と一致します。一方、非対称な分布では大きくずれます。代表的な分布の `E[X]` 公式は [代表的な確率分布](../probability-distributions/) のノートにまとめてあります。
...
左の正規分布では `E[X]` がピーク（最頻値）と一致します。一方、右の対数正規分布では「裾」の長い右側に期待値が引っ張られ、最頻値より大きい位置に来ます。期待値は外れ値の影響を強く受ける、というのが [平均と中央値](../median/) の議論と同じ構造です。実用ではロバスト性が必要なときに中央値や trimmed mean を併用します。

---

### 期待値の重要な性質

期待値の使いやすさは、次の 2 つの代数的性質に支えられています。
...
線形性は「`X` と `Y` が独立でなくても成り立つ」という点が重要です。共分散があっても、和の期待値は期待値の和になります。
...
3 つのヒストグラムが `X`、`Y`、`X + Y` のサンプル分布で、それぞれの期待値の位置に赤い破線を引いてあります。右図の `X + Y` の平均と、`X` と `Y` の平均の和が一致しています（赤と黒のラインがほぼ重なる）。`X` と `Y` は相関を持つように作ってあります。期待値の線形性は独立性を要求しないため、それでも等号が成り立ちます。

この性質のおかげで、複雑な確率モデルの期待値も「項ごとに分解して足し算」で扱えるケースが多いです。例として、ニューラルネットの損失 `E[Σ_i ℓ_i]` は `Σ_i E[ℓ_i]` に分解でき、ミニバッチ平均でも有効な期待値推定になります。

---

### 条件付き期待値と total expectation

ある変数の値を固定したときのもう片方の期待値を条件付き期待値（conditional expectation, `E[Y | X = x]`）と呼びます。回帰の問題設定そのもので、「`x` が与えられたときの `y` の期待値」を学習することが回帰モデルの目的になります。

全期待値の法則（law of total expectation, tower property）が便利な道具となります。

`E[Y] = E[E[Y | X]]`

「`X` を固定して条件付き期待値を計算 → さらにその `X` についての期待値を取る」と、無条件の期待値が復元できる、という構造です。階層的なデータ（クラスタ × サンプル）を扱うときや、ベイズ推論で事後期待値を求めるときに頻出します。
...
期待値は機械学習の式の文法のような存在で、ほとんどの目的関数や評価式が `E[...]` の形をしています。

---

### 適さないケース / 落とし穴

- 期待値が存在しないケース: 裾の重い分布（コーシー分布など）では `E[X]` が発散する。サンプル平均は値を取る。ただし、何回繰り返しても収束しない
- 期待値が実用的な代表値にならないケース: 二峰性の強い分布では期待値が「どちらの山にも属さない谷」を指すことがある（[平均](../mean/) のノート参照）
- サンプル平均は期待値の「無偏推定量」。ただし、分散の推定では `(1/n) Σ (x_i - x̄)^2` に偏りがあり、`(1/(n-1)) Σ (x_i - x̄)^2` の補正が必要になる
- 期待値とサンプル平均の混同: 研究や論文では両者の区別が重要
- 線形性は独立を必要としない。一方、積の期待値 `E[X Y]` は独立性が必要。共分散がある場合 `E[X Y] = E[X] E[Y] + Cov(X, Y)`
