...
IQR は「中央 50% のデータが収まる幅」を表します。[標準偏差](../stddev/) と並んで散らばりを測る指標として使われるが、IQR は外れ値の影響を受けにくいという特徴を持ちます（順位ベースの量なので、極端な値が混ざっても変わらない）。
...
「1.5」という係数は経験則で、厳しめに見るなら `3.0`（extreme outlier）を使います。正規分布の場合、`1.5×IQR` ルールで誤検出される割合はおおよそ 0.7% 程度になることが知られており、3σ ルール（0.27%）よりやや甘い目安となります。
...
赤い線が中央値の予測、緑の帯が `q=0.1` 〜 `q=0.9` の 80% 予測区間です。データの分散が `x` の右側で大きくなる構造（不均一分散）をしているため、緑の帯も右に行くほど広がっています。在庫計画・需要予測・SLA 設計のように「平均だけでなくバラつきを含めて意思決定したい」場面で頻出する手法で、`q=0.95` のような片側の予測値を「安全マージン」として使います。
...
- 分布の要約（代表値とばらつき）
- 外れ値の判定（IQR = Q3 - Q1）  
IQR（Interquartile Range）は、Q3とQ1の差で、中央50%の広がりを示します。`IQR = Q3 - Q1` と定義し、外れ値判定の目安にも使います。
- データの比較（A/Bの分布差）
