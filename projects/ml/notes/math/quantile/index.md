...
四分位点（quartile）は、データを小さい順に並べて「4 等分する位置」の値を取り出した代表値です。[中央値](../median/) を「2 等分する位置」と捉えると、四分位点はその拡張になっています。

- 第 1 四分位点（Q1）: 下位 25% の位置の値
- 第 2 四分位点（Q2）: 50% の位置の値（[中央値](../median/) と同じ）
- 第 3 四分位点（Q3）: 下位 75% の位置の値

より一般化したものが「分位点（quantile）」で、「データの中で何 % の位置にあるか」を任意の % で指定できます。四分位点は分位点の `25%`, `50%`, `75%` の特別な場合と言えます。

### 用語整理: quantile / percentile / quartile

似た用語が複数あって混乱しやすいので整理しておきます。
...
| percentile（百分位点） | 100 等分（1%〜99%） | 試験成績の上位何 %、検査値の参照範囲 |
| quantile（分位点） | 任意の比率 `q ∈ [0, 1]` | `q = 0.95` で 95% 点 |

機械学習や統計の文献では「quantile」が最も一般的な呼び方で、`np.quantile(data, 0.25)` のような任意比率の指定に対応しています。

### IQR（四分位範囲）と外れ値判定

四分位点の応用で最も重要なのが IQR（Interquartile Range、四分位範囲）です。

```text
IQR = Q3 - Q1
```

IQR は「中央 50% のデータが収まる幅」を表します。[標準偏差](../stddev/) と並んで散らばりを測る指標として使われます。その一方で、IQR は外れ値の影響を受けにくいという特徴を持ちます（順位ベースの量なので、極端な値が混ざっても変わらない）。

外れ値判定の標準的なルール（Tukey の方法）は IQR を使います。

- 下側の外れ値: `x < Q1 - 1.5 * IQR`
- 上側の外れ値: `x > Q3 + 1.5 * IQR`

`1.5` という数字は経験則で、より厳しく判定したい場合は `3.0` を使います。3σ ルールが正規分布前提なのに対し、IQR ルールは分布形を問わずに使えるロバストな手段と言えます。

### 箱ひげ図の構成

四分位点を視覚化する代表的な手段が箱ひげ図（boxplot）です。
...
箱ひげ図 1 枚で「中央値・四分位範囲・外れ値の有無」が同時に分かるので、複数群の比較（A/B、カテゴリ別の値分布）で常用されます。
...
Tukey の IQR ルールを適用して、外れ値を「マークするだけ」のシンプルな検出を見ます。
...
緑の帯が IQR、赤い破線が `Q1 - 1.5·IQR` と `Q3 + 1.5·IQR` の境界（フェンス, fence）で、その外側に落ちた点が外れ値として ×印で示されています。注入した 5 個の極端な値が全部正しくフラグされています。

正規分布の場合、`1.5×IQR` ルールで誤検出される割合はおおよそ 0.7% 程度で、3σ ルール（0.27%）よりやや甘い目安となります。

### 分位回帰で「予測区間」を作る

分位回帰（quantile regression）は、平均ではなく特定の分位点（10%、50%、90% など）を予測する回帰手法です。低分位と高分位の予測を上下に並べると、予測区間（prediction interval）として読めます。
...
赤い線が中央値の予測、緑の帯が `q=0.1` 〜 `q=0.9` の 80% 予測区間です。データの分散が `x` の右側で大きくなる構造（不均一分散）をしているため、緑の帯も右に行くほど広がっています。在庫計画・需要予測・SLA（Service Level Agreement、サービス品質の合意）設計のように「平均だけでなくバラつきを含めて意思決定したい」場面で頻出する手法で、`q=0.95` のような片側の予測値を「安全マージン」として使います。

---

### 数学での使いどころ

数学・統計の文脈では、四分位点や分位点は以下で使われます。

- 分布の要約（代表値とばらつき）
- 外れ値の判定（IQR = Q3 - Q1）
- データの比較（A/Bの分布差）

数学的には、分位点は「小さい順に並べたときの位置」を数式で書いたものだと考えます。
...
四分位点と IQR は前処理・特徴量設計・評価の各段階で頻出します。

- 特徴量のスケール感の把握: EDA（Exploratory Data Analysis、探索的データ分析）で `describe()` の Q1/Q2/Q3 を見る
...
- 不均衡データの閾値調整: 予測確率の Q3 や 90% 点（`q=0.9`）を「異常の閾値」として使う
