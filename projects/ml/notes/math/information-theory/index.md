...
[ベイズの定理](../bayes-theorem/) と [対数・指数関数](../log-exp-logodds/) の知識を前提として、確率分布を「情報量」という単位で扱う視点を得るためのノートとなります。[損失関数](../../ml/loss-functions/) の交差エントロピー、[決定木](../../ml/decision-tree/) の情報利得、[特徴量重要度](../../ml/feature-importance/) の MDI など、ML の多くの場面で前提として登場します。

### 自己情報量: 珍しい事象ほど情報が大きい

確率 `p(x)` で起こる事象 `x` が観測されたときの「驚き」を量化したものが自己情報量です。

`I(x) = -log_2 p(x)`

底に 2 を取ると単位は「ビット（bit）」、自然対数なら「ナット（nat）」となります。機械学習では自然対数を使うのが標準です。
...
「めったに起きないこと」が起きたら、その情報量が大きい、というのが情報理論の出発点です。事前確率を予測している話との対比で「予想していた事象が起きてもニュース性は低く、予想外の事象が起きるとニュース性は高い」という日常感覚と一致します。

---

### エントロピー: 平均情報量

確率変数 `X` の取りうる全ての値 `x` について、自己情報量を確率で重み付けして平均したものがエントロピーです。

`H(X) = -Σ_x P(x) log P(x)`

これが「`X` を予測するときの平均的な不確実性」あるいは「`X` を符号化するのに必要な平均ビット数」を表します。

2 値ベルヌーイ分布（コイン投げ）のエントロピーを `p` の関数として描きます。
...
「クラスが半々」が最も予測しにくい状態、というのは [決定木](../../ml/decision-tree/) で分割の不純度として登場する Gini と本質的に同じ構造です。決定木は「分割によって `H` が下がる量（情報利得）」が最大の特徴量を選んでいます。
...
連続分布の中でエントロピー最大なのは、(1) 値域が `[a, b]` で固定なら一様分布、(2) 平均と分散が固定なら正規分布、というのが「最大エントロピー原理」として知られます。「分かっている制約だけ仮定し、それ以上の情報は仮定しない」という発想で、ベイズ統計の事前分布の選択に利用されます。
...
の式で定義されます。「`Q` を `P` の近似と思って使ったときの符号化の余分なビット数」「`P` で発生したサンプルを `Q` で説明したときの誤差」と解釈できます。
...
mild:      KL(P || Q) = 0.154 bits
strong:    KL(P || Q) = 0.743 bits
```

![同じ Q（一様）に対して P が離れるほど KL が増える](./info_kl_divergence.svg)

3 つのパネルで `Q`（赤、一様分布）は固定、`P`（青、真の分布）を徐々に偏らせています。`P` と `Q` が一致するとき KL = 0、`P` が偏るほど KL が増えます。「真分布 `P` に対してモデル `Q` がどれだけ wrong か」を定量化する尺度として、機械学習の評価・学習に頻出します。

---

### 交差エントロピー = エントロピー + KL

[損失関数](../../ml/loss-functions/) のノートで扱った交差エントロピーは、エントロピーと KL に分解できます。

`H(P, Q) = -Σ_x P(x) log Q(x) = H(P) + KL(P || Q)`

ここで `H(P)` は真分布 `P` のエントロピーで、訓練データが決まれば固定です。`KL(P || Q)` がモデル `Q` 依存の部分で、「交差エントロピーを最小化する = KL を最小化する = モデル `Q` を真分布 `P` に近づける」という関係になります。
...
と表されます。これが「`Y` を知ることで `X` の不確実性がどれだけ減るか」、あるいは「`X` と `Y` の同時分布が独立分布の積からどれだけずれているか」を測る量となります。

`I(X; Y) = KL(P(x, y) || P(x) P(y))`

の形でも書けて、「[同時分布](../joint-marginal-conditional/) が周辺分布の積（= 独立を仮定したもの）からどれだけ離れているか」と読めます。独立なら `I = 0`、強く依存していれば `I > 0`です。

![情報ダイアグラム: H(X), H(Y), I(X;Y)](./info_mutual_information.svg)

ベン図的に書くと、`X` のエントロピー `H(X)` と `Y` のエントロピー `H(Y)` の重なり部分が相互情報量です。重なりが大きいほど 2 変数の依存が強いことを示します。

[相関係数](../correlation/) が測れるのは線形依存だけです。一方、相互情報量は線形・非線形を問わず一般的な依存性を捉えます。[特徴量選択](../../ml/feature-selection/) の `mutual_info_classif` は、この値を特徴量の有用性スコアとして使っています。
...
- 変分推論（VAE）: ELBO（Evidence Lower Bound、変分下界）の中で KL ダイバージェンスが事前分布との近さを表す
...
scikit-learn では `metrics.mutual_info_score`、`metrics.normalized_mutual_info_score`、`feature_selection.mutual_info_classif` などが提供されています。
