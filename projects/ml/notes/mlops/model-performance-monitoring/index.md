...
モデル性能劣化の監視（model performance monitoring）は、本番運用しているモデルの予測品質を継続的に観測し、劣化を早期に検知する仕組みです。学習時の精度は本番でずっと維持されるわけではなく、[データドリフト](../data-drift/) や仕様変更で時間とともに落ちていきます。劣化に気づかず放置すると、ユーザー体験やビジネス指標がじわじわ悪化し、最悪の場合は気づいたときには手遅れになります。

監視は単一の指標ではなく、複数の層（infra → data → model → business）を組み合わせて行うのが標準です。検知後の対応として [再学習パイプライン](../retraining-pipeline/) が自動起動するように設計しておくのが理想形となります。

### 4 つの監視レイヤ

監視対象は粒度の違う 4 つのレイヤに整理できます。それぞれ「気づきたい問題」「責任者」「使う指標」が違います。
...
「モデルの精度が落ちた」と一口に言っても、原因がモデル本体か、入力データか、配信基盤か、上位のビジネス事情かで対処が違います。4 層を別々に観測しておくと切り分けが速くなる、と考えられます。

---

### モデル性能指標の時系列監視

最も基本的な監視が「精度の時系列プロット」。生の値だと daily noise が大きいので、移動平均（moving average, MA）と閾値を組み合わせるのが定番です。
...
灰色の点が日次の生値、青線が 7 日移動平均です。橙の破線（warning, 0.88）と赤の破線（critical, 0.85）が SLO（Service Level Objective、満たすべき品質目標）の閾値で、移動平均が critical を下回った時点で on-call にページが飛びます。最初の 75 日は緩やかなドリフト、その後外的要因（例: 新キャンペーンによるユーザー層変化）で一気に劣化、というシナリオで、閾値超過まで約 80 日かかっています。

「critical までいかなくても warning は早めに鳴らす」というのが鉄則です。warning で再学習や原因調査を開始すれば、critical に達する前に手が打てます。

---

### Ground truth が遅れる問題と proxy metrics

精度を測るには ground truth（実際の正解）が必要です。ただし、現実には数時間〜数週間遅れて到着することが多いです。例として、レコメンドの「クリックの有無」は数秒で分かります。一方、「購入したかどうか」は数日後、「リテンションに寄与したか」は数週間後にしか確定しません。
...
ground truth を待っていたら検知が遅すぎるので、proxy metrics（代理指標）で「劣化の兆候」を早期に捉える設計にします。
...
複数 proxy のうち 1 つでも異常を示せばアラート、というのが standard pattern となります。

---

### 入力ドリフト検知: KS 検定と PSI

ground truth がほぼ取れないケースでも、入力データの分布変化は即時検知できます。代表的な指標が KS 検定（Kolmogorov-Smirnov test）と PSI（Population Stability Index）。

- KS 検定: 訓練分布と本番分布の累積分布関数の最大差を測る。`0` で完全一致、`1` で完全不一致
- PSI: 分布を 10 ビンに分けて、訓練分布と本番分布の KL ダイバージェンスを両方向で計算して足し合わせる。`< 0.1` で安定、`0.1〜0.25` でやや変化、`> 0.25` で大きく変化
...
左の KS 統計量と右の PSI が、60 日目あたりから上昇を始めています。前章の accuracy 監視では 75 日目まで影響が見えなかったのに対し、入力ドリフトはおよそ 15 日早く兆候を検知できています。「予測精度が落ちる前に入力分布の変化を捉える」のがドリフト監視の意義となります。

ただし入力ドリフトと精度劣化は必ずしも 1:1 対応しません。`P(x)` が変わっても `P(y|x)` が安定なら精度は維持されます（covariate shift, 詳細は [データドリフト](../data-drift/) 参照）。入力ドリフトはあくまで「何かが起きている」のシグナルで、それが実害になっているかは精度監視で確認する、という二段構えになります。

---

### コード例: Prometheus + Grafana で監視メトリクスを公開

推論サーバー側で predict のたびにメトリクスを記録し、Prometheus がスクレイプして Grafana で可視化する標準パターンです。
...
Grafana のダッシュボードで「過去 24 時間の P99 latency」「予測スコアの分布の変化」「入力特徴量の平均推移」を並べておけば、人間が眺めるだけでもおかしさに気づきやすいです。アラートルール（PromQL: `histogram_quantile(0.99, ...) > 0.2`）を入れれば自動通知も組めます。

---

### Alert の二段階設計

監視で「常にアラートが鳴っている」状態は実質「鳴っていない」のと同じになります。閾値とアラート設計は次のように二段階にします。
...
例として「accuracy SLO = 0.85」のモデルなら、warning = 0.88、critical = 0.85 で設定です。warning でじわじわ落ちる兆候を早めに捉え、critical で即時対応します。

「false alarm が多すぎる」は監視の最大の敵で、誤検知が続くと on-call が無視するようになります。閾値・移動平均ウィンドウ・連続違反数（例: 「3 連続で閾値を下回ったらアラート」）の調整で false alarm を抑える運用ノウハウが必要となります。

---

### Observability の 3 本柱: metrics / logs / traces

完全な observability は metrics だけでは作れません。
...
機械学習特有の「予測の根拠を後から追えるようにしたい」要求（規制対応、顧客クレーム対応）に対しては、logs に「リクエスト ID、モデル version、入力特徴量、予測値、確率」をすべて記録するのが必須です。サンプリングで全件は厳しいので、1% 〜 10% 程度の sampling rate で十分な情報量が得られます。
...
- 集約指標だけ見て segment 別を見ない: 全体精度は維持されていても、特定セグメント（新規ユーザー、特定地域）で激落ち、というケースを見落とす
