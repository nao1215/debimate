...
赤い線が再学習無し、緑が 30 日ごとに再学習したケースです。1 年後には再学習無しは accuracy 0.55、再学習あり 0.88 前後で 30% 以上の差がつきます。黒い破線が SLO（service level objective）の 0.85 で、再学習無しは半年で SLO を割り、再学習ありはギリギリ維持できています。
...
Feature store は規模が小さいうちはオーバーキルだが、特徴量数 100 を超え、複数モデルで共有する段階になると無いと苦しい、と考えられます。
