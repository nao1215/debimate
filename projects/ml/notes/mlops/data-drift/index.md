...
[過学習](../../ml/overfitting/) や [データリーク](../../ml/data-leakage/) が学習時の不具合であるのに対し、ドリフトは運用後にしか起きません。MLOps（機械学習の運用工学）の中心課題のひとつで、検知と再学習の仕組みを最初から設計に織り込む必要があると言えます。

### 4 種類のドリフトに分解できる

ドリフトは「何が変化したか」で 4 種類に分かれます。それぞれ対処の打ち手が違うため、まずどれが起きているかを切り分けることが出発点になります。
...
確率分布の言葉で書くと整理しやすいです。`P(x)` は入力分布、`P(y|x)` は入力が与えられたときの目的変数の条件付き分布、`P(y)` は目的変数の周辺分布です。どれが動いたかで呼び名と対処が変わります。

---

### Covariate shift（共変量シフト）

入力分布 `P(x)` が変わる一方、入力と出力の関係 `P(y|x)` は変わらないパターンです。例として、訓練データのユーザー年齢分布は若年中心だったところ、サービスの認知が広がって本番では年配層が増えた、といったケースが該当します。
...
入力の統計量（平均・分散）を訓練時と本番でモニタリングすれば検知できることが多いです。検出方法としては Kolmogorov-Smirnov 検定（連続値の分布が同じかを判定する統計検定）や PSI（Population Stability Index, ビン分けした確率分布の差分指標）が定石です。

---

### Concept drift（概念ドリフト）

入力分布 `P(x)` は変わっていません。その一方で、入力と出力の関係 `P(y|x)` が変わるパターンです。同じユーザー特徴量を入力しても、出るべき正解ラベルが変わってしまいます。
...
ユーザー行動の変化（流行・季節性・社会変化）、競合サービスの動き、業界規制の変更などが原因になりやすいです。例えば、レコメンドモデルで「以前は同ジャンル過去購入が強いシグナルだったところ、最近は価格帯が効くようになった」というケースは典型的な concept drift です。

入力の統計量だけ見ても検知できないのが厄介と言えます。検知には予測精度そのものを本番で監視する必要があり、ground truth（正解ラベル）が遅れて入手できる場合は遅延評価ダッシュボードの整備が必須となります。

---

### Label shift（ラベルシフト / Prior shift）

目的変数の周辺分布 `P(y)` が変わるパターンです。例として、不正取引検知モデルで「不正の発生率が当初 1% だったところ、攻撃キャンペーンの影響で 5% に上昇した」場合、入力分布も入出力関係も変わっていないのに、陽性比率（プリオール）の変化だけでモデルの確率出力の校正がずれます。

検知は単純で、本番での陽性比率を継続的に計測すれば良いです。対処も比較的軽く、確率出力の事後校正（Platt scaling や isotonic regression の再フィット）で済むことが多いです。再学習までは不要なケースが多いと言えます。

---

### Domain shift（ドメインシフト）

`P(x)` `P(y|x)` `P(y)` のいずれか、または複数が同時に変化する広義の概念です。「訓練データを取った環境と本番の環境が違う」全般を指します。先述の 3 種が単独で起きることは少なく、現実には複合的に進行します。

切り分けは「先に統計検定で `P(x)` を確認」「次に予測精度と陽性比率を確認」「最後にラベルとの関係を確認」の順で進めるのが効率的です。

---

### 検知する方法

ドリフト検知は、変化を測る対象別に手段が分かれます。
...
evidently / nannyml / Fiddler / Arize といったオープンソース・SaaS ツールが、上記の検定と可視化をまとめて提供しています。自前で構築せずツール導入で済ませる選択肢もあります。

---

### 対処: 再学習と新陳代謝

検知だけでは精度は戻りません。実際の対処は次のようなパターンに分かれます。
...
4. オンライン学習
    ...
    - 連続的に変化するデータには有効。ただし、暴走リスクの管理が必要
5. アンサンブル切り替え
...
定期再学習で SLA（Service Level Agreement, ここでは「精度 0.70 を割らない」という運用上の約束）を守る運用パターンを示しています。再学習周期の決め方は「ドリフトの速さ × ビジネス許容度」で決まると考えられます。

---

### 機械学習での使いどころ

ドリフト概念は、モデル単体ではなく「運用パイプラインを設計するとき」に効いてきます。
...
一般に、デプロイがゴールではなく「劣化に気づける状態」を作ることがモデルを本番で使い続ける前提条件と考えられます。コードもモデルも変えていないのに精度が下がる現象がある、と知っているだけで運用後の混乱を減らせます。
