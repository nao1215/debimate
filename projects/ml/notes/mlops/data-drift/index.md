...
入力分布 `P(x)` が変わるが、入力と出力の関係 `P(y|x)` は変わらないパターンです。例として、訓練データのユーザー年齢分布が若年中心だったが、サービスの認知が広がって本番では年配層が増えた、といったケースが該当します。
...
入力分布 `P(x)` は変わっていないが、入力と出力の関係 `P(y|x)` が変わるパターンです。同じユーザー特徴量を入力しても、出るべき正解ラベルが変わってしまいます。
...
ユーザー行動の変化（流行・季節性・社会変化）、競合サービスの動き、業界規制の変更などが原因になりやすいです。レコメンドモデルで「以前は同ジャンル過去購入が強いシグナルだったが、最近は価格帯が効くようになった」といったケースは典型的な concept drift です。
...
目的変数の周辺分布 `P(y)` が変わるパターンです。例として、不正取引検知モデルで「不正の発生率が当初 1% だったが、攻撃キャンペーンの影響で 5% に上昇した」場合、入力分布も入出力関係も変わっていないのに、陽性比率（プリオール）の変化だけでモデルの確率出力の校正がずれます。
