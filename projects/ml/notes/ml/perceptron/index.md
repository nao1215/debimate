...
[線形回帰](../linear-regression/) や [ロジスティック回帰](../logistic-regression/) と数式の中身はほぼ同じで、最後の出力を「シグモイドで確率に変換するか / step function で 0,1 に変換するか」だけが違います。発想は古典的です。それでも、(1) 単純な構造で動く、(2) パーセプトロン収束定理という強い理論保証がある、(3) 多層に積めば任意の関数を近似できる（[多層パーセプトロン](#多層パーセプトロンと-xor)）、という 3 点で深層学習の出発点として今でも教科書の最初に登場します。

### 単純パーセプトロンの構造

入力 → 重み付き和 → ステップ関数 → 出力、という最小構成です。

![単純パーセプトロンと多層パーセプトロンの構造](./perceptron_architecture.svg)

左が単純パーセプトロン: 入力 `x_1, x_2, x_3` に重み `w_1, w_2, w_3` を掛けて加算し、step function で `0/1` を出します。右が多層パーセプトロン（multi-layer perceptron, MLP）: 入力層と出力層の間に「隠れ層」を 1 層挟み、非線形な活性化関数を経由します。

学習則（perceptron learning rule）は、誤分類したサンプル `x_i` について次のように重みを更新します。
...
データが線形分離可能なら、有限回の更新で必ず収束します（パーセプトロン収束定理）。これは強い保証です。その一方で、後述の通り「線形分離可能でなければ収束しない」のが弱点となります。

---

### 学習の様子: 重みが更新で動く

2 次元の線形分離可能データに対して学習を回したときの境界の動きを見ます。
...
灰色の細い線が初期〜途中の境界、緑の太線が最終境界です。初期重み `w = 0` から始まり、誤分類した点を 1 つずつ拾って重みを修正していきます。15 epoch で線形分離可能なこのデータには十分収束しており、緑の境界が 2 群を正しく分けています。

[ロジスティック回帰](../logistic-regression/) のように勾配ベースで全データの損失を一気に下げるのではなく、「間違えた点だけを 1 つずつ修正していく」のがパーセプトロン学習則の特徴です。確率的勾配降下法（[SGD](../../math/gradient-descent-sgd/)）の最古の例とも言えます。

---

### 線形分離可能性と XOR 問題

パーセプトロンの最大の弱点は「線形分離可能なデータしか学習できない」ことです。AND・OR は 1 本の直線で分けられます。一方、XOR はそれができません。
...
左の AND と中央の OR は 1 本の緑線で 0 と 1 を分けられます。右の XOR は、どこに直線を引いても (0,0) と (1,1) を同じ側に、(0,1) と (1,0) を反対側に置くことができません。赤い破線で 2 本試した結果も、4 点を正しく分類できていません。

この XOR 問題は 1969 年に Minsky と Papert が指摘したもので、「パーセプトロンは XOR すら解けない」という結論が AI 冬の時代（1970-80 年代）の引き金の 1 つになりました。解決策が見つかるまで約 20 年を要し、その解決策が次の多層パーセプトロンと [誤差逆伝播法](../backpropagation/) です。

---

### 多層パーセプトロンと XOR

XOR は単層では解けません。ただし、「隠れ層を 1 層挟んで非線形な [活性化関数](../activation-functions/) を入れる」と解けます。中間層が「入力空間を歪めて新しい特徴量を作る」役割を果たすため、出力層から見ると線形分離可能な空間に変換されます。
...
色付きの領域が決定領域で、4 点がそれぞれ正しいクラス領域に入っています。決定境界は曲がっており、単層では引けない形になっています。

普遍近似定理（universal approximation theorem）は「十分な数のユニットを持つ 1 隠れ層 MLP は、任意の連続関数を任意の精度で近似できる」と主張します。これが「ニューラルネットは何でも学習できる」と言われる理論的根拠で、現代の深層学習はこの原理の延長線上にあります。
...
パーセプトロンとロジスティック回帰は出力部分だけが違う近い親戚関係にあります。

- パーセプトロン: `ŷ = step(z)`、学習則は「誤分類サンプルだけ更新」
- ロジスティック回帰: `p = sigmoid(z)`、損失は交差エントロピー、勾配は全サンプルから

歴史的にはパーセプトロンが先で、その確率版がロジスティック回帰、その多層版がニューラルネット、と発展してきました。

### 数学での使いどころ

- パーセプトロン収束定理: 線形分離可能なら有限回で収束（[線形代数](../../math/vector-matrix-ops/) の幾何的議論）
- 線形分離可能性の判定: VC 次元（Vapnik-Chervonenkis 次元、分類器がどれだけ複雑なラベル配置まで分けられるかを表す指標）、マージン理論
...
現代では単純パーセプトロンを単体で使う場面はほぼありません。学習素材として、または以下の派生形として登場することが多いです。
...
実装は scikit-learn の `Perceptron` か、PyTorch の `nn.Linear` と `torch.heaviside` の組み合わせです。実用上は MLP（多層）として使うのがほぼすべてとなります。
...
- 重み初期化が単純すぎる（全部 0 のままなど）: 学習が遅い、対称性が崩れない。He / Xavier 初期化が標準
