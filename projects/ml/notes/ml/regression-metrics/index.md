...
[損失関数](../loss-functions/) のノートで触れた通り、学習時の損失関数と評価指標は別に選べます。MSE で学習しても評価は R² で見る、というのが普通です。ただし、同じ評価データと同じサンプル重みで測る限り、MSE を小さくするほど R² は大きくなり、両指標によるモデルの順位は一致します。
...
`SS_res = Σ (y_i - ŷ_i)^2` は残差の二乗和、`SS_tot = Σ (y_i - ȳ)^2` はターゲットの全変動（平均値からのずれの二乗和）（ベースラインモデル「平均値だけ予測する」との比較）。

R² = 1 で完全一致、R² = 0 で「平均だけ予測するベースライン」と同等、R² < 0 で「ベースラインより悪い」を意味します。テストデータで R² が負なら、平均値を返すベースラインよりも予測性能が悪いことになります。過学習、訓練時との分布の違い、外挿、特徴量不足、モデルの不適合などを疑います。
...
予測 vs 正解の散布図（true vs predicted）に対して、3 指標を計算する例です。
...
赤い破線が `y = ŷ`（完全予測）の対角線で、点がそれにどれだけ近いかが視覚的な精度の目安です。RMSE と MAE は同じ単位（ここでは `y` のスケール）で「平均してこれくらいズレている」を表し、R² は「ベースラインを基準とした相対的な良さ」を表します。

R² が高い（≥ 0.9）からといって RMSE が低いとは限らない点に注意が必要です。`y` の分散が大きいタスクでは、絶対誤差が大きくても R² が高くなる、ということがあります。

---

### RMSE と MAE の違い: 外れ値感度

RMSE は二乗を取るため、大きい残差を相対的に重く罰します。同じデータでも「外れ値が 1 つ入る」だけで RMSE と MAE が大きく乖離します。
...
クリーンなデータでは RMSE と MAE の比は約 1.25（正規分布なら理論値 `√(π/2) ≈ 1.25`）。外れ値 1 つで RMSE が 2.91 倍に膨らむのに対し、MAE は 1.49 倍にしか動きません。RMSE と MAE の比が正規分布を仮定した目安である約 1.25 より大きい場合、一部に大きな残差があると疑う材料になります。ただし、この比だけで外れ値の存在を断定することはできません。裾の重い誤差分布、異分散、一部セグメントだけでの大きな予測失敗でも比は大きくなります。
...
R² は「ベースライン（平均値予測）に対してどれだけ分散を説明したか」の比率です。式の中身を分解します。

`R² = 1 - SS_res / SS_tot`

- `SS_tot`: 真のターゲットの全変動（平均値からのずれの二乗和）
- `SS_res`: モデルが残した残差の二乗和（平均値ではなく予測値からのずれの二乗和）

`SS_tot` が「何もしない場合の誤差」、`SS_res` が「モデルが残した誤差」で、その比が小さいほど R² は 1 に近づきます。
...
左の図ではターゲットの全分散（緑線 = 平均からの広がり）から、モデルの予測との残差（灰色縦線）を引いた残りを「モデルが説明した分散」として読めます。右の棒グラフでこの分解が定量化されており、`SS_tot = SS_reg + SS_res` が成立します（OLS、最小二乗法で当てはめた場合）。R² が `SS_reg / SS_tot` と等価なのもこの構造から導かれます。

注意点として、R² は無次元で「タスク間で比較しやすい」反面、サンプル数が少なすぎたり特徴量が多すぎると過大評価されやすいです。`adjusted R²`（自由度補正版）も併用すると安全と考えられます。

---

### MAPE: 相対誤差 (%) で比較する

MAE は単位を持つため、`y` のスケールが極端に違うタスク間で比較できません。例として、「気温予測の MAE = 1.0」と「売上予測の MAE = 1000」は字面では比べられません。

MAPE（mean absolute percentage error）は誤差を `y` の何 % かに変換します。
...
両方とも `±5%` のノイズを乗せたデータです。それでも MAE は規模に比例して、 3.5 → 3400 と変化するのに対し、MAPE はどちらも `~4.79%` で揃います。スケールの違うタスクを横断比較するなら MAPE が便利です。ただし、いくつかの落とし穴があります。

- `y` が 0 近傍だと `|y_i|` で割って爆発する（数値が `inf` になる）
- `y_i = 0` のサンプルでは定義できない
- 予測の上振れと下振れを非対称に評価する（`y = 100, ŷ = 50` のずれ率は 50%、`y = 50, ŷ = 100` は 100%）

これらを避けるため、SMAPE（symmetric MAPE）や RMSLE（root mean squared log error）を併用するのが一般的です。
...
- `mean_squared_error(y, ŷ)`、`root_mean_squared_error(y, ŷ)`（RMSE。`squared=False` は scikit-learn 1.4 で非推奨、1.6 で削除）
...
- MAPE で `y_i = 0` のサンプル: 定義できないので除外するか、SMAPE / WAPE（weighted absolute percentage error、誤差の合計を実測値の合計で割る指標）に切り替える
- 評価指標と学習目的の不一致: MSE 学習で MAE 評価、のような場合、最良モデルが学習指標的に最良とは限らない。揃えるか、別途分位回帰で MAE 学習する
- 「R² = 0.95 だから良い」「R² = 0.5 だから悪い」の絶対判定: タスクの難易度・データの SNR（signal-to-noise ratio、信号対雑音比）で R² の良し悪しは変わる。同じタスクの過去モデルや baseline との比較で見る
