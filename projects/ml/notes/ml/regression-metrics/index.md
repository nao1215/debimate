...
左の図ではターゲットの全分散（緑線 = 平均からの広がり）から、モデルの予測との残差（灰色縦線）を引いた残りを「モデルが説明した分散」として読めます。右の棒グラフでこの分解が定量化されており、`SS_tot = SS_reg + SS_res` が成立します（OLS の場合）。R² が `SS_reg / SS_tot` と等価なのもこの構造から導かれます。
...
両方とも `±5%` のノイズを乗せたデータだが、MAE は規模に比例して 3.5 → 3400 と変化するのに対し、MAPE はどちらも `~4.79%` で揃います。スケールの違うタスクを横断比較するなら MAPE が便利だが、いくつかの落とし穴があります。
