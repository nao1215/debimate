...
[損失関数](../loss-functions/) のノートで触れた通り、学習時の損失関数と評価指標は別物として扱えます。MSE で学習しても評価は R² で見る、というのが普通であり、両者が一致しないことの方がむしろ多いです。
...
R² = 1 で完全一致、R² = 0 で「平均だけ予測するベースライン」と同等、R² < 0 で「ベースラインより悪い」を意味します。テストデータで R² が負になるのは「モデルが平均値予測すら下回る」という危機的な状況で、過学習やデータリークの疑いが強いです。
...
クリーンなデータでは RMSE と MAE の比は約 1.25（正規分布なら理論値 `√(π/2) ≈ 1.25`）。外れ値 1 つで RMSE が 2.91 倍に膨らむのに対し、MAE は 1.49 倍にしか動きません。「RMSE と MAE の比が 1.25 より大きく離れているなら、データに外れ値が含まれる可能性が高い」という診断的な使い方もできます。
