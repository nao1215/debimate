...
もう 1 系統のリークは、前処理パイプラインの組み方によって生まれる。[標準化](../standardization/) の `StandardScaler.fit` をデータ全体に適用してから train / test に分けると、テストデータの平均・分散の情報が train の前処理に紛れ込む。同様に、特徴量選択や次元削減（[PCA](../pca/) など）をデータ全体に対して先に行うと、テストの分布情報が漏れる。
