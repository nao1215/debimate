...
kNN（k近傍法、k-Nearest Neighbors）は、新しい点を予測するとき、訓練データの中で「その点に最も近い k 個のサンプル」を見て、多数決（分類）または平均（回帰）で答えを決める手法です。  
モデルを学習で作るのではなく、訓練データをそのまま記憶しておくのが特徴です。「怠惰な学習（lazy learning）」とも呼ばれます。

距離は通常ユークリッド距離 `d(x, x_i) = sqrt(sum((x - x_i)^2))` を使います。スケール差が結果に直結するため、[標準化](../standardization/)（[標準偏差](../../math/stddev/)で割って平均 0・分散 1 に揃える）が基本です。距離計算は内積を経由した [ベクトル・行列の演算](../../math/vector-matrix-ops/) で書け、`||x - x_i||^2 = ||x||^2 - 2 x · x_i + ||x_i||^2` の関係から行列積 1 回で全訓練点との距離を一括計算できます。
...
- 訓練時間はゼロに近い。その代わり、推論時に全訓練点との距離計算が必要
...
まず kNN の予測がどう決まるかを概念図で見ます。星印が予測したい点で、点線が最も近い5点（k=5）への距離です。この5点のラベルの多数決で予測クラスが決まります。
...
k の値で決定境界がどう変わるかを以下のコードで確認します。k=1 は訓練点1つだけ見るので境界がギザギザで[過学習](../overfitting/)気味になります。k=25 は遠くまで見るので境界がなだらかになる一方、細かい構造が潰れます。一般には[交差検証](../cross-validation/)で適切な k を選ぶと考えられます。
...
- [カーネル密度推定](../../math/kde/)の単純な形（k を固定して密度を見積もる）

数学的には、kNNは「[平均](../../math/mean/)を局所的に取る」発想に近いです。点ごとに近傍だけで集計するため、グローバルな分布に依存しないのが特徴です。

---

### 機械学習での使いどころ

機械学習では、kNN は以下で使われます。
