...
[決定木](../decision-tree/) 単体は過学習しやすく不安定（高 Variance）だが、独立に近い多数の木を平均することで Variance を縮める、というのが原理です（[バイアス-バリアンス分解](../bias-variance-tradeoff/) の bagging 側）。学習後は `feature_importances_`（MDI）で各特徴量の寄与が取れるが、高カーディナリティバイアスがあるため [特徴量重要度](../feature-importance/) で再検証するのが安全と考えられます。
