...
過学習（overfitting）は、モデルが訓練データの特徴を「覚えすぎ」て、未知のデータでうまく予測できなくなる現象です。  
訓練データではほぼ満点なのに、テストデータや本番データでは精度が大きく落ちる、という形で表に出ます。

逆に、モデルがデータの傾向を捉えきれずに訓練でもテストでも精度が低い状態を未学習（underfitting）と呼びます。良いモデルは「ちょうど良い複雑さ」でこの2つの間に位置します。

### 過学習に気づく方法

最初に頼るシグナルは、訓練データとテストデータの精度差（汎化ギャップ）です。
...
汎化ギャップは1つの分割で判断するとブレるので、本番では[交差検証](../cross-validation/)で複数分割の平均を取って判断します。

---

### モデルの複雑さと過学習

多項式回帰で次数（複雑さ）を変えた3例を比較します。同じデータに対して、表現力の違うモデルを当てた結果を並べたものです。
...
degree=15 のモデルは訓練データ上では誤差ほぼゼロです。しかし、x=-2 や x=2 付近のように訓練点から少し外れると暴れ、未知の点を渡されると外します。これが過学習の正体です。

---

### 訓練誤差と検証誤差のカーブ

モデルの複雑さを少しずつ上げながら訓練誤差と検証誤差を追うと、典型的なパターンが現れます。

![train vs validation error across complexity](./overfitting_loss.svg)

- 訓練誤差はモデルを複雑にするほど下がり続ける（極端に複雑にすればゼロにもできる）
- 検証誤差はある複雑さまでは下がる。そこを超えると上がり始める
- 検証誤差が最小になる点が「ちょうど良い複雑さ」

この「U 字の底」を探す作業がモデル選択の本質です。検証誤差を見ずに訓練誤差だけ追いかけると、必ず過学習側に倒れます。
...
過学習対策はいくつかの方向に分かれます。優先順位は状況によります。まずは軽い対策から試すのが定石です。
...
scikit-learn で訓練誤差と検証誤差を出して汎化ギャップを見る最小例です。
...
deep モデルは訓練 1.00 / テスト 0.85 で汎化ギャップが大きい一方、shallow モデルはギャップが縮みテスト精度も上がっています。「複雑＝強い」ではないことが端的に見えます。

---

### 機械学習での使いどころ

過学習は「現象の名前」であって特定のアルゴリズムに紐づきません。あらゆる教師あり学習で常に意識する必要があります。
...
一般に、訓練精度とテスト精度の両方を必ず出して差を確認する習慣をつけると、出てきた数字がそのまま使えるかどうか判断しやすくなると言えます。
...
- 過学習対策はモデル側だけの問題ではない: [データのリーク](../data-leakage/)（未来情報の混入など）が本質原因のことも多い
- 本番運用後の精度劣化は過学習と紛らわしいが別物: 学習時の現象と、本番運用後に入力やラベルの関係が変わる [データドリフト](../../mlops/data-drift/) は原因も対処も違う
