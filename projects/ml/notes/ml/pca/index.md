...
数学的な中核は「共分散行列の[固有値分解](../../math/eigen-decomposition/)」で、固有ベクトルが主成分の方向、固有値がその軸上での分散を表す。データ行列を低次元に写す操作は [行列ベクトル積](../../math/vector-matrix-ops/) `Z = X V` の形をしており、線形代数の道具がそのまま PCA の実装になる。
