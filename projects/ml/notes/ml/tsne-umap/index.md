...
- PCA（左）: 数字によって雲がやや分かれるものの、相互に重なる。線形射影なので「直交した変換」しかできない
...
「埋め込み上で見える距離」は元データの距離と直接対応しない、というのは t-SNE / UMAP のアルゴリズム上の性質（局所構造優先）から来ます。「同じクラスタに属する点同士は近い」は信用できます。一方、「異なるクラスタの距離」は信用できないと考えるのが安全です。

UMAP は t-SNE よりは大局構造を保ちます。それでも「距離の絶対値」を信頼できるレベルではありません。距離の比較が本当に必要なら PCA や MDS（Multidimensional Scaling）を使います。

### 数学での使いどころ

- KL ダイバージェンス最小化 ([情報理論](../../math/information-theory/) との接続)
- t 分布の重い裾（crowding 問題への対処）
...
- 巨大なデータでそのまま実行: t-SNE は厳密法が `O(n²)`、Barnes-Hut 法でも `O(n log n)` かかる。UMAP の方が大規模に強い
- スケールが揃わない: 標準化を必ず先に
