...
線形回帰（linear regression）は、入力特徴量の線形和でターゲットを予測する最も基本的な教師あり回帰モデルです。

`ŷ = w_1 x_1 + w_2 x_2 + ... + w_d x_d + b = w · x + b`

この単純さにもかかわらず、線形回帰は機械学習の中核に座っています。`(1)` 解析解 `θ = (X^T X)^-1 X^T y` が閉形式で出る、`(2)` 係数の解釈が直接できる（`x_k` が 1 増えると `y` が `w_k` 増える）、`(3)` [正則化](../regularization/) と組み合わせて Ridge / Lasso に拡張できる、という 3 点で実務的にも理論的にも汎用的に扱えます。

[ロジスティック回帰](../logistic-regression/) や [サポートベクターマシン](../svm/)、ニューラルネットの 1 層分も「線形和 + 何か」の構造を持つので、線形回帰の挙動を押さえておくと後続の手法の見通しが良くなります。
...
の予測式に対して、[平均二乗誤差（MSE）](../loss-functions/) を最小化する `w, b` を求めるのが OLS（ordinary least squares, 最小二乗法）です。
...
の形で出ます（バイアス項 `b` は特徴量に常 1 の列を加えれば同じ式に含められる）。`(X^T X)` が逆行列を持つことが前提で、特徴量同士の相関が極端に強い（[多重共線性](#多重共線性と正則化)）と数値的に不安定になります。

実装上は `LinearRegression()` が裏で QR 分解または SVD（Singular Value Decomposition、特異値分解）で `(X^T X)^-1` を直接計算せずに解きます。これにより数値安定性が確保されます。

---

### 単回帰の見た目

最も単純な「特徴量 1 つ vs ターゲット」の散布図に OLS を当てる例です。
...
赤い線が OLS の推定直線、灰色の縦線が各サンプルの残差（`y_i - ŷ_i`）で、これらの二乗和が最小になるように直線が引かれます。緑の点線が真の関係 `y = 0.7 x + 1.5` で、ノイズの中で OLS がそれをほぼ言い当てています。

直線の傾き `w` は「`x` が 1 増えると `y` が `w` 増える」という直接の解釈ができ、これが線形モデルの最大の利点でもあります。決定木やニューラルネットでは同じ解釈が成り立たないため、説明性が要求される場面で線形モデルが選ばれることが多いです。

---

### 線形モデルの「表現力」と複雑度

「線形」は重み `w` について線形であって、特徴量については任意の変換を施せます。多項式特徴量 `x, x^2, x^3, ...` を加えれば曲線も表現できる（多項式回帰）が、次数を上げすぎると過学習します。
...
左の degree=1 は直線で、波打つ真の関係（緑点線）を表現できず、汎化性能も悪いです（under-fit）。中央の degree=3 が真の関数 `sin(2πx)` の形をうまく捉えています。右の degree=15 は訓練点の 1 つ 1 つに張り付き、見るからに過学習しています（[過学習](../overfitting/) のノートと同じ構図）。

「特徴量を増やせば表現力は上がる。ただし、過学習しやすくなる」というトレードオフは、線形モデルでも非線形モデルでも共通する課題で、[バイアス-バリアンス分解](../bias-variance-tradeoff/) と [交差検証](../cross-validation/) で次数や [正則化](../regularization/) の強さを選ぶのが定石です。

---

### 残差プロットで仮定をチェックする

OLS の理論的な前提として「残差が等分散（homoscedastic）」「残差が独立」「残差が正規分布」があります。残差プロット（横軸: 予測値、縦軸: 残差）の形を見ると、これらの前提が崩れていないかが分かります。
...
上段（良い例）の右パネルでは残差が予測値全域で均一に 0 の周りに散らばっており、OLS の前提が満たされています。下段（悪い例）では予測値が大きい右側で残差が大きく広がる「ファン型」になっており、不均一分散（heteroscedasticity）の症状です。この場合、信頼区間が信頼できなくなり、係数の検定結果も歪みます。対策は次のいずれか。

- 目的変数を `log y` で変換する（[歪度](../../math/skewness/) のノート参照）
- 重み付き最小二乗法（WLS）で分散の逆数を重みにする
- 分散の大きい区間を別モデルで扱う

残差プロットは線形回帰を実務で使うとき最初に描く図のひとつ、と考えてよいです。

---

### 多重共線性と正則化

特徴量同士の相関が高い場合、`(X^T X)` がほぼ特異になり、具体的には、係数が極端な値を取る、符号が反転する、標準誤差が爆発するという不安定さが出ます。これを多重共線性（multicollinearity）と呼びます。

対策の一つが [正則化](../regularization/) で、係数の大きさを抑制するペナルティ項を損失に加えます。
...
赤い線が「真に効く 5 個の特徴量」の係数、灰色の線が「真に効かない 15 個の特徴量」の係数で、横軸が正則化の強さ `α` です。
...
`α` をどれくらいに設定するかは [交差検証](../cross-validation/) で選ぶのが標準です。`α = 0` で素の OLS、`α` を大きくしすぎると過学習を抑える代わりに under-fit します。
...
- 因果関係の主張: 線形回帰の係数は「相関」を示すだけで、操作変数や RCT（Randomized Controlled Trial、ランダム化比較試験）などの設計が無いと「`x` を増やすと `y` が動く」とは言えない
