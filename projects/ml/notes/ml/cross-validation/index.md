...
交差検証（cross validation, CV）は、限られた学習データを「訓練用」と「検証用」に何通りも分け直して評価することで、モデルの性能をより安定して見積もる手法です。  
1 回だけの分割では「たまたま簡単／難しい分割を引いた」せいで結果がブレるので、分割の組み合わせを変えながら平均を取って判断します。

[過学習](../overfitting/)を検出する道具であり、[正則化](../regularization/)の `alpha` のような[ハイパーパラメータ](../hyperparameter/)を安定的に選ぶ道具でもあります。

### 最もよく使う K-Fold

データを `k` 等分（fold）し、毎回 1 つを検証用、残り `k-1` 個を訓練用に使います。これを `k` 回繰り返して、各回の検証スコアを平均します。`k=5` または `k=10` が定番です。

![5-fold cross validation の分割](./cross_validation_split.svg)

赤いブロックがその回の検証データ、青が訓練データです。`fold 1` から `fold 5` まで検証スライスをずらしていき、得られた 5 つのスコアの平均をモデル評価値とします。
...
[正則化](../regularization/)の `alpha` のように「強さ」を持つハイパーパラメータは、訓練データだけ見て決められません（訓練誤差は `alpha=0` で最小になり常に過学習側を選んでしまう）。CV を使い、各候補で `k` 回の検証スコアを平均し、最も良いものを選びます。

![Ridge の alpha を CV で選ぶ](./cross_validation_alpha.svg)

縦の点線が CV スコアの最大点です。`alpha` が小さすぎると過学習で平均スコアが下がり、大きすぎると未学習で下がります。中間にスイートスポットがあり、CV で見つけにいきます。エラーバーは fold 間のばらつきで、これが大きい候補は「平均は良くても安定しない」と判断できます。

---

### Python での実例

scikit-learn は `cross_val_score` と `GridSearchCV` が主要 APIです。
...
CV を使うと検証データを明示的に分けなくて済みます。ただし、テストデータは必ず別に確保しておきます（CV 内で何度もスコアを見て選んだモデルは、CV スコア自体も少し楽観的に出るため）。
...
一般に、単なる train/test 分割ではなく CV で評価する方が、出てきた数字を信用しやすくなると考えられます。`cross_val_score` で 5-fold の平均と[標準偏差](../../math/stddev/)を出すのが定番の最小構成です。
