...
特徴量選択（feature selection）は、使える特徴量の中からモデルにとって有用な部分集合を選び出し、それ以外を捨てる前処理です。目的は (1) [過学習](../overfitting/) の抑制、(2) 学習・推論コストの削減、(3) モデルの説明性向上、(4) [次元の呪い](../curse-of-dimensionality/) の緩和、の 4 点に集約されます。
...
[特徴量重要度](../feature-importance/) と密接に関連します。ただし、重要度は「学習済みモデルの解釈」、特徴量選択は「学習前の特徴量の絞り込み」という違いがあります（実際には重要度ベースの選択もあるので境界はあいまい）。
...
最も軽量な手法です。分散がほぼゼロ（= 全サンプルでほぼ同じ値）の特徴量はモデルへの寄与が薄いので捨てます。
...
赤い棒（分散 < 0.05）が閾値以下で除外される特徴量、緑が保持される特徴量です。`VarianceThreshold(threshold=0.0)` で「全く動かない定数列」だけ落とすのが安全です。閾値を厳しくすると本物の特徴量も落ちる危険があるので、低めに設定するのが現実的と考えられます。

注意点: 標準化していない生のスケールに依存するので、[標準化](../standardization/) との順序で結果が変わります。普通は VarianceThreshold を先に当てます。

---

### Filter 法 2: SelectKBest（F 検定・相互情報量）

統計的な指標で「目的変数との関係が強い」上位 `k` 個を残します。代表的なスコアは次の通りです。
...
F 検定（青）と相互情報量（MI、緑）は、どちらも「予測に効く特徴量を上位に出す」点では共通します。その一方で、F 検定は線形関係に特化し、MI は非線形依存も拾います。両者がほぼ一致するなら関係が線形的、大きく食い違うなら非線形成分が強い、と読めます。

`SelectKBest(score_func=f_classif, k=10).fit_transform(X, y)` のように 1 行で上位 `k` 個を取り出せます。

---

### Wrapper 法: RFE（recursive feature elimination）

RFE は「全特徴量で学習 → 最も重要度の低い特徴量を 1 つ削除 → 残りで再学習」を繰り返し、最終的に指定した数の特徴量に絞ります。`RFECV` は [交差検証](../cross-validation/)（CV, cross-validation）と組み合わせて「精度が最大になる特徴量数」を自動探索します。
...
横軸が「残した特徴量数」、縦軸が CV スコアの平均（青の帯は標準偏差）。少ない特徴量だと表現力不足で under-fit、増やすと頭打ちになり、最終的にノイズが入ってわずかに下がります。赤い破線が CV スコアが最大の点で、ここを「最適な特徴量数」として採用します。

RFE は精度が高い方法です。ただし、(特徴量数 × 評価回数) のオーダーで計算量が増えるため、特徴量が数百を超えると現実的でなくなります。中規模データでは強く効く手法です。ただし、いつも使えるわけではないと考えられます。

---

### Embedded 法: Lasso と Tree importance

学習自体が特徴量選択を兼ねるアプローチです。代表は Lasso（L1 [正則化](../regularization/)）と木系モデルの組み込み重要度です。
...
左の Lasso は L1 で多くの係数を 0 に潰し、真の informative 特徴量（緑、最初の 8 つ）を中心に残しています。右の RF は重要度が連続値で、informative はおおむね高く出るがノイズの一部にも値が入ります。
...
実装上は `SelectFromModel(LassoCV()).fit_transform(X, y)` や `SelectFromModel(RandomForestClassifier(), threshold="median").fit_transform(X, y)` のように、しきい値で自動的に切ることができます。

---

### 多重共線性対策: 相関の強い特徴量を 1 つにまとめる

[相関係数](../../math/correlation/) のノートで触れた通り、相関の強い特徴量は (1) 線形モデルの係数を不安定にする、(2) ツリーの分割で重要度が分散する、という問題を起こします。相関行列を計算して、しきい値（例: `|r| > 0.9`）を超えるペアの片方を捨てるのも有効な特徴量選択になります。
...
これは [PCA](../pca/) との対比で見ると分かりやすいです。PCA は相関のある特徴量を線形結合で集約して新しい軸を作るのに対し、こちらは元の特徴量から代表 1 つを残します。解釈性を優先するなら後者、精度を優先するなら PCA / Lasso のどちらかが現実的となります。
...
- 特徴量エンジニアリングの反復: 仮説で作った特徴量が effective かを定量化
...
- 次元削減（PCA）と混同: PCA は新しい軸を作るので元の特徴量が消える。一方、特徴量選択は元のまま部分集合を残す。説明性目的なら特徴量選択、情報集約目的なら PCA、と用途が違う
