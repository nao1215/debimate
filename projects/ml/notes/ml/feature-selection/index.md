...
[特徴量重要度](../feature-importance/) と密接に関連するが、重要度は「学習済みモデルの解釈」、特徴量選択は「学習前の特徴量の絞り込み」という違いがあります（実際には重要度ベースの選択もあるので境界はあいまい）。
...
F 検定（青）と MI（緑）はどちらも「予測に効く特徴量を上位に出す」点では共通するが、F 検定は線形関係に特化し、MI は非線形依存も拾います。両者がほぼ一致するなら関係が線形的、大きく食い違うなら非線形成分が強い、と読めます。
...
RFE は「全特徴量で学習 → 最も重要度の低い特徴量を 1 つ削除 → 残りで再学習」を繰り返し、最終的に指定した数の特徴量に絞ります。`RFECV` は CV と組み合わせて「精度が最大になる特徴量数」を自動探索します。
...
RFE は精度が高いが、(特徴量数 × 評価回数) のオーダーで計算量が増えるため、特徴量が数百を超えると現実的でなくなります。中規模データでの「ガチで効く手法」だが、いつも使えるわけではないと考えられます。
