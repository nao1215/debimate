...
両者とも `p = 0.5`（クラスが半々で最も予測しにくい）で最大値を取り、`p = 0` または `p = 1`（全部同じクラス = 不純度ゼロ）で 0 になります。形は似ているが、Entropy の方が `p = 0` 付近でわずかに尖っており、Gini の方が計算が軽いです（log 計算が要らない）。実用上は両者で性能差はほぼ無く、scikit-learn のデフォルトは Gini です。

回帰木では不純度の代わりに「分散」を最小化します（分割後のグループ内の `y` の分散の和が最小になるように分割する）。これは MSE 損失で OLS をやっているのと同じ発想で、[損失関数](../loss-functions/) の選び方の話と直結します。
